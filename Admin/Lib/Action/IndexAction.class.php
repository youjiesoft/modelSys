...
//        $dandianyonghu=Cookie::get('dandianyonghu');
//        $ssomap['dandianyonghu']=$dandianyonghu;
//        $ssodata=M('single_sign_on')->where($ssomap)->field('username,password')->find();
//        $_SESSION['username'] = 'admin';
//        $_SESSION['password'] = 'hndhnd20171127';
...
// 	}
	...
	*/
	...
	//值更新获取夸表取数和存储过程
	function valeditfunc(){
        $res['status'] = 0;
        $res['msg'] = '';
		try {
            $gtype = trim($_REQUEST['gtype']);
            if ($gtype == '') {
            ...
            //跨表取数
            if ($gtype == 'tables') {
                $sql = trim($_REQUEST['sql_str']);
                if (empty($sql)) {
                    throw new Exception("SQL不能为空");
                }
                // 实例化一个空模型，没有对应任何数据表
                $list = $Dao->query(html_entity_decode($sql));
                if ($list) {
                    if(count($list)>0&&count($list[0])>0){
                        $data=trim(current($list[0]));
                        if($data!=''){
                        	$res['status']=1;
                        	$res['data']=$data;
            ...
            //存储过程
            if ($gtype == 'sp') {
            	$spname=trim($_REQUEST['spname']);
            	if($spname==''){
                    throw new Exception("存储过程为空");
				}
                $sql = " CALL {$spname};";
