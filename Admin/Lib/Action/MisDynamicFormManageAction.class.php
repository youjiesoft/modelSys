...
// 			$this->createAutoAnameConfig($anameList[$nodename],$nodename);
		...
		if($_REQUEST['type']==1){
		   ...
		   if($ischoise){
			...
			try{
				...
				$transeModel->commit();
				$this->success("操作成功", '', $resultReturn);
	...
	private function autoUpdate(){
		...
		//表记录mas表
		$MisDynamicDatabaseMasDao=M("mis_dynamic_database_mas");
		$mastablename = $MisDynamicDatabaseMasDao->where("modelname='{$this->nodeName}'")->getField("tablename");
		$mastablename =str_replace(' ','',ucwords(str_replace('_',' ',$mastablename)));
		//表记录sub表
		$MisDynamicDatabaseSubDao=M("mis_dynamic_database_sub");
		//内嵌表顺序
		$mis_dynamic_form_datatable_sortModel = M("mis_dynamic_form_datatable_sort");
		//查询内嵌表最大序号
		$where = array();
		$where['modelname'] = $this->nodeName;
		$datatable_sort = $mis_dynamic_form_datatable_sortModel->where($where)->order("sort desc")->getField("sort");
		if(!$datatable_sort)$datatable_sort = 0;
...
// 			$newupdatefield[$yk] = array_filter(array_merge($updatefield1,$updatefield2));
		...
		foreach ($tablename as $tkey=>$tval){
			...
			foreach ($fieldname[$tkey]  as $key=>$val){
				...
				$subDate['modelname'] = $this->nodeName;
				//当前字段名称等于数据源字段名称
				...
				if($_POST['ename'][$tkey]){ //修改原有表信息
					...
					if(!$_POST['oldfieldname'][$tkey][$key]){
						...
						if($subDate['category']=="datatable"){
							//新增字段为内嵌表
							++$datatable_sort;
							$subsort = array();
							$subsort['modelname'] = $this->nodeName;
							$subsort['title'] = $subDate['title'];
							$subsort['field'] = $subDate['field'];
							$subsort['tablename'] = $mastablename."Sub".ucfirst($subDate['field']);
							$subsort['sort'] = $datatable_sort;
							$subsotresult = $mis_dynamic_form_datatable_sortModel->add($subsort);
							if($subsotresult===false){
								//$msg = "表字段记录内容写入信息失败！".$mis_dynamic_form_datatable_sortModel->getLastSql();
								$msg = "错误信息如下".$mis_dynamic_form_datatable_sortModel->getDbError();
								throw new NullDataExcetion($msg);
							}
						}
...
function editAutoForm() {
	...
	private function createJsText($nodeNames='' , $tplType=''){
		...
		if($nodeName=='MisAutoJwf'){
		    $box .="\r\n\tvar nbpw=$('form#MisAutoJwf_add').find('#nbpw').val();";
            $box .="\r\n\tvar nbpwname=$('form#MisAutoJwf_add').find('input[name="."neibupingweiname"."]').val();";
            $box .="\r\n\t$('form#MisAutoJwf_add').find('div.field_canhuirenyuan').find('input[type="."text"."]').val(nbpwname);";
            $box .="\r\n\t$('form#MisAutoJwf_add').find('div.field_canhuirenyuan').find('input[name="."canhuirenyuan"."]').val(nbpw);";
        }
//        if (in_array('iframe',$_POST['catalog'])) {
//            $box .="\r\n\tvar username ='admin';";
//            $box .="\r\n\tvar password ='hndgs201804';";
//            $box .="\r\n\tjQuery.ajax({";
//            $box .="\r\n\turl:\"http://1.62.154.90:37799/WebReport/ReportServer?op=fs_load&cmd=sso\",";
//            $box .="\r\n\tdataType:\"jsonp\",";
...
EOF;
		...
		$formid = getFieldBy($nodeName, "actionname", "id", "mis_dynamic_form_manage");
		if($formid){
			$addjsstr.=$this->valeditjs($formid);
			$editjsstr.=$this->valeditjs($formid);
			//加入查看页面值更新事件 gml 2017/9/12
			$viewjsstr.=$this->valViewJs($formid);
			$editjsstr.=<<<EOF
...
EOF;
		...
		/*值更新公共js*/
		$addjsstr.=$this->valeditcommonjs();
		$editjsstr.=$this->valeditcommonjs();
		//gml添加测试为了满足查看页面的iframe
		//$viewjsstr.=$this->valeditcommonjs();
		/*值更新公共js*/
...
// 		throw new NullDataExcetion('我是出错 ， 出来'.$retSql);
	...
	private function insertDatebase($alist,$misDaddId,$tplname='index'){
		...
		$where = array();
		$where['modelname'] = $this->nodeName;
		$sort = $mis_dynamic_form_datatable_sortModel->where($where)->order("sort desc")->getField("sort");
		if(!$sort)$sort = 0;
		//字段主键记录
...
// 			$MisDynamicFormTemplateModel->commit();
		...
		foreach ($alist as $k1=>$v1){
			...
			foreach ($v1['list'] as $k3=>$v3){
				...
				$subdata['modelname']=$this->nodeName;
				unset($subdata['id']);
				...
				//判断是否存在内嵌表字段
				if($v3['category']=="datatable"){
					++$sort;
					$subsort = array();
					$subsort['modelname'] = $this->nodeName;
					$subsort['title'] = $v3['title'];
					$subsort['field'] = $v3['field'];
					//获取表名
					$mastablename =str_replace(' ','',ucwords(str_replace('_',' ',$v1['tablename'])));
					$subsort['tablename'] = $mastablename."Sub".ucfirst($v3['field']);
					$subsort['sort'] = $sort;
					$subsotresult = $mis_dynamic_form_datatable_sortModel->add($subsort);
					if(!$subsotresult){
						$msg = "表字段记录内容写入信息失败！".$mis_dynamic_form_datatable_sortModel->getLastSql();
	...
	foreach ($tablenameArr[1] as $key => $val) {
		...
		if( $this->isaudit){
			$detailList['auditState']=array(
					...
					'sortnum' =>(count($detailList)+1),'searchField'=>$this->tableName.".auditState",
					'transform'=>'0',
					'unique'=>'0',
					'validate'=>'0',
					'required'=>'0'
		...
		$detailList['operateid']=array(
				...
				'sortname' => 'operateid',
				'sortnum' => (count($detailList)+1),
				'searchField'=>$this->tableName.".operateid",
				'transform'=>'0',
				'unique'=>'0',
				'validate'=>'0',
				'required'=>'0'
		...
		$detailList['companyid']=array(
				...
				'sortnum' => (count($detailList)+1),
				'searchField'=>$this->tableName.".companyid",
				'transform'=>'1',
				'unique'=>'0',
				'validate'=>'0',
				'required'=>'0',
		...
		$detailList['projectid']=array(
				...
				'searchField'=>$this->tableName.".projectid",
				'transform'=>'0',
				'unique'=>'0',
				'validate'=>'0',
				'required'=>'0'
		...
		$detailList['createid']=array(
				...
				'table' => 'user',
				'sortnum' => (count($detailList)+1),
				'searchField'=>$this->tableName.".createid",
				'transform'=>'1',
				'unique'=>'0',
				'validate'=>'0',
				'required'=>'0',
		...
		$detailList['createtime']= array(
				...
				'fieldtype' => 'date',
				'fieldcategory' => 'date',
				'searchField' => $this->tableName.".createtime",
				'conditions' => '',
				'type' => 'time',
				'issearch' => '0',
				'isallsearch' => '0',
	...
	private function getTagsListincData(&$detailList ,$v1, $notshowcontroll ,$k1, $shownumber , &$shownumberCount,$sort){
		...
		switch ($v1[$controllProperty['catalog']['name']]){
			...
			case 'areainfo':
				...
				break;
			case 'datatable':
                		$detailList[$k1]['istwodimensional']     = $v1['istwodimensional'];
                		$detailList[$k1]['istwodimensionalshow'] = $v1['istwodimensionalshow'];
                		break;
			case 'textarea':
		...
		try{
			if(is_array($temp)){
				if($temp['id']){
					...
					//表记录mas表
					$MisDynamicDatabaseMasDao=M("mis_dynamic_database_mas");
					$masinfo =$MisDynamicDatabaseMasDao->where("tablename = '{$tablename}'")->field("formid,modelname")->find();
					$tableid = $masinfo['formid'];
					$nodename = $masinfo['modelname'];
					$tablesubData=array(
							'masid'=>$tableid,
					...
					if($iscreate){
						...
						if($subid){
							//判断是否存在内嵌表字段
							if($tablesubData['category']=="datatable"){
								//内嵌表顺序
								$mis_dynamic_form_datatable_sortModel = M("mis_dynamic_form_datatable_sort");
								//查询内嵌表最大序号
								$where = array();
								$where['modelname'] =$nodename;
								$datatable_sort = $mis_dynamic_form_datatable_sortModel->where($where)->order("sort desc")->getField("sort");
								if(!$datatable_sort)$datatable_sort = 0;
						
								++$datatable_sort;
								$subsort = array();
								$subsort['modelname'] = $nodename;
								$subsort['title'] = $tablesubData['title'];
								$subsort['field'] = $tablesubData['field'];
								//获取表名
								$mastablename =str_replace(' ','',ucwords(str_replace('_',' ',$tablename)));
								$subsort['tablename'] = $mastablename."Sub".ucfirst($tablesubData['field']);
								$subsort['sort'] = $datatable_sort;
								$subsotresult = $mis_dynamic_form_datatable_sortModel->add($subsort);
								if(!$subsotresult){
									$msg = "表字段记录内容写入信息失败！".$mis_dynamic_form_datatable_sortModel->getLastSql();
...
//============================超级无敌霸气华丽的分割线==========================================
	 ...
	 *
	 */
	public function valeditcontroll(){
		try {
			F('valedit_data',array());
			$formid=$_REQUEST['formid'];//表单id
			$propertyid=$_REQUEST['propertyid'];//组件属性id
			if(empty($formid)){
				throw new Exception('formid错误，请关闭窗口重试！');
			}
			if(empty($propertyid)){
				throw new Exception('property错误，请关闭窗口重试！');
			}
			$propertyM=M("mis_dynamic_form_propery");
			$ppMap['id']=$propertyid;
			$property=$propertyM->where($ppMap)->find();
//			var_dump($property);
			//查询所有可赋值控件
			$kfzMap['formid']=$formid;
			$kfzMap['status']=1;
			$kfzMap['_string'] = 'isrichbox<>1 or isrichbox is null';
			$fieldtypes=array('text', 'select', 'radio', 'checkbox', 'textarea','iframe');
			$kfzMap['category']=array("in",$fieldtypes);
			$kfzp=$propertyM->where($kfzMap)->field("id,fieldname,category,title")->select();
			$valedit_data=array('formid'=>$formid,'propertyid'=>$propertyid);
			F('valedit_data',$valedit_data);

			//查询值更新规则
			$mverM=M('mis_value_edit_rules');
			$mvermap['formid']=$formid;
			$mvermap['property_id']=$propertyid;
			$mvermap['status']=1;
			$mvers=$mverM->where($mvermap)->order("my_sort asc")->select();

			//操作类型 值更新 漫游
            $edit_type=0;
			if($mvers){
				foreach($mvers as $k=>$v){
					//逻辑规则
					$ljrule=json_decode($v['lj_rules'],true);
					if(!empty($ljrule)){
						$ljkeys="";
						foreach($ljrule as $k1=>$v1){
							$ljkeys[]=$k1;
						}
						$ljkeys=implode(",",$ljkeys);
						$mvers[$k]['lj_key']=$ljkeys;
					}
					//结果规则
					$jgrule=json_decode($v['jg_rules'],true);
					if(!empty($jgrule)){
						$jgkeys="";
						foreach($jgrule as $k1=>$v1){
							$jgkeys[]=$k1;
						}
						$jgkeys=implode(",",$jgkeys);
						$mvers[$k]['jg_key']=$jgkeys;
					}
					F('valedit_rules_'.$formid.'_'.$k.'_1',$ljrule);
					F('valedit_rules_'.$formid.'_'.$k.'_2',$jgrule);
					$acts=json_decode($v['act_obj'],true);
					$mvers[$k]['act_obj_name']=$acts[1];
				}
                $edit_type=$v['edit_type'];
				$this->assign("mvers",$mvers);
				$this->assign("is_rules",1);
			}else{
				for($i=0;$i<50;$i++){
					F('valedit_rules_'.$formid.'_'.$i.'_1',array());
					F('valedit_rules_'.$formid.'_'.$i.'_2',array());
				}
			}

			$this->assign('ve_type',$edit_type);
			$this->assign('kfzp',$kfzp);
	 ...
	 * 值更新规则列表设置-操作
	 */
	public function setvaleditcontroll(){
		$model=M('mis_value_edit_rules');
		$model->startTrans();
		try{
			$formid=$_REQUEST['formid'];//表单id
			$propertyid=$_REQUEST['propertyid'];//控件id
			$ve_type=$_REQUEST['ve_type'];//触发类型
			$ve_rules_sort=$_REQUEST['ve_rules_sort'];//序列号数组
			$ve_rules=$_REQUEST['ve_rules'];//逻辑规则内容
			$op_obj=$_REQUEST['op_obj'];//操作对象
			$op_type=$_REQUEST['op_type'];//操作类型
			$op_res=$_REQUEST['op_res'];//操作结果 是否
			$ve_res_rules=$_REQUEST['ve_res_rules'];//操作结果规则内容
			$ve_res_par=$_REQUEST['ve_res_par'];//结果规则参数
			$ve_par=$_REQUEST['ve_par'];//逻辑规则参数
			$my_sorts=$_REQUEST['ve_rules_my_sort'];//强制排序
			if(empty($formid)){
				throw new Exception('表单id错误');
			}
			if(empty($propertyid)){
				throw new Exception('控件id错误');
			}
			if(empty($ve_type)){
				throw new Exception('触发类型错误');
			}
			if(empty($ve_rules_sort)){
				throw new Exception('请添加规则');
			}
			$i=1;
			//先删除该表单该字段的规则
			$dmap['formid']=$formid;
			$dmap['property_id']=$propertyid;
			if(!$model->where($dmap)->delete()){
				throw new Exception('删除原数据失败');
			}
			$rule_ids=array();
			foreach($ve_rules_sort as $key=>$val){
			    if($my_sorts[$val]==''){
                    throw new Exception('第'.$i.'条未设置排序');
                }
				if(empty($op_obj[$val])){
					throw new Exception('第'.$i.'条未选择操作对象');
				}
				if(empty($op_type[$val])){
					throw new Exception('第'.$i.'条未选择操作类型');
				}
				if(in_array($op_type[$val],array(3,4,5))&&$op_res[$val]==''){
					throw new Exception('第'.$i.'条必须选择结果设置');
				}
				if(in_array($op_type[$val],array(1,2,6))&&empty($ve_res_rules[$val])){
					throw new Exception('第'.$i.'条必须设置结果规则');
				}
				$data=null;
				$ljrule=F('valedit_rules_'.$formid.'_'.$val.'_1');
				$jgrule=F('valedit_rules_'.$formid.'_'.$val.'_2');
				$ljrules="";
				$jgrules="";
				if($ve_par[$val]){
					$pars=explode(',',$ve_par[$val]);
					foreach($pars as $k=>$v){
						$ljrules[$v]=$ljrule[$v];
					}
				}
				if($ve_res_par[$val]){
					$ve_pars=explode(',',$ve_res_par[$val]);
					foreach($ve_pars as $k=>$v){
						$jgrules[$v]=$jgrule[$v];
					}
				}
				...
				$data['edit_type']=$ve_type;
				$data['act_type']=$op_type[$val];
				$data['act_obj']=json_encode(explode("||",$op_obj[$val]));
				$data['lj_rules']=$ljrules&&!empty($ve_rules[$val])?json_encode($ljrules):"";
				$data['lj_content']=$ve_rules[$val];
				$data['sort']=$val;
				$data['jg_content']=$ve_res_rules[$val];
				$data['is_act']=$op_res[$val];
				$data['jg_rules']=$jgrules&&!empty($ve_res_rules[$val])?json_encode($jgrules):"";;
				$data['status']=1;
				$data['my_sort']=empty($my_sorts[$val])?0:$my_sorts[$val];
				$data['create_time']=time();
				$rid=$model->add($data);
				if(!$rid){
					throw new Exception('第'.$i.'条设置失败');
				}

				F('valedit_rules_'.$formid.'_'.$val.'_1',array());
				F('valedit_rules_'.$formid.'_'.$val.'_2',array());
				$rule_ids[]=$rid;
				$i++;
			}
			$rule_ids=implode(',',$rule_ids);
			$model->commit();
			$this->ajaxReturn($rule_ids,'设置成功',1,'json');
		}catch(Exception $e){
			$model->rollback();
			$this->ajaxReturn(null,$e->getMessage(),0,'json');
		}
	}
	/**
	 * 规则设置
	...
	public function setrules(){
		try {
			$valedit_data=F('valedit_data');
			$sort = $_REQUEST['sort'];
			$contr = $_REQUEST['contr'];
			$rcon = trim($_REQUEST['rcon']);
			$sort = trim($sort);
			if (!isset($_REQUEST['sort'])||$sort=='') {
				throw new Exception('序列号错误');
			}
			if (empty($contr)) {
				throw new Exception('目标标签错误');
			}
			$formid=$valedit_data['formid'];//表单id
			if(empty($formid)){
				throw new Exception('id错误，请重试');
			}
			$valedit_data['sort']=$sort;
			$valedit_data['contr']=$contr;
			F('valedit_data',$valedit_data);

//			F('valedit_rules_'.$formid.'_'.$sort.'_'.$contr,array());
			$datas=F('valedit_rules_'.$formid.'_'.$sort.'_'.$contr);

			$pars=$this->getpars($datas);

			$this->assign('pars',$pars);

			$this->assign('ve_sort',$sort);//列表排序
			$this->assign('contr',$contr);//规则类型

			$this->assign('rcon',$rcon);//规则内容

			$this->assign('formid',$valedit_data['formid']);
			$this->assign('propertyid',$valedit_data['propertyid']);
	 ...
	 * 控件值设置
	 */
	public function controlval(){
		try {
			$valedit_data=F('valedit_data');
			$formname=$_REQUEST['myform'];//当前表单名和dialog的key
			$parentform=$_REQUEST['parentform'];//父级表单名
			$parentctr=$_REQUEST['parentctr'];//父级需要写入的控件name属性
			$formid=$valedit_data['formid'];//表单id
			if(empty($formid)){
				throw new Exception('id错误，请重试');
			}
			if(empty($formname)||empty($parentform)||empty($parentctr)){
				throw new Exception('系统错误，请关闭窗口重试');
			}
			//查询可取值控件
			$propertyM = M("mis_dynamic_form_propery");
			$kqzMap['formid'] = $formid;
			$kqzMap['status'] = 1;
			$fieldtypes = array('text', 'select', 'radio');
			$kqzMap['category'] = array("in", $fieldtypes);
			$kqzp = $propertyM->where($kqzMap)->field("id,fieldname,category,title")->select();

			$par_name=$_REQUEST['par_name'];
			$sort=$valedit_data['sort'];//序列号id
			if(!isset($valedit_data['sort'])||$sort==''){
				throw new Exception('序列号错误，请重试');
			}
			$cty=$valedit_data['contr'];//控件类型
			if(empty($cty)){
				throw new Exception('结果类型错误，请重试');
			}
			$data=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
			if(!empty($par_name)){
				$this->assign("ct_name",$data[$par_name]['rules'][1]);
			}
			$this->assign("par_name",$par_name);

			$this->assign("kqzp",$kqzp);

			$this->assign('formid',$valedit_data['formid']);
			$this->assign('propertyid',$valedit_data['propertyid']);

			$this->assign('formname',$formname);
			$this->assign('parentform',$parentform);
			$this->assign('parentctr',$parentctr);
	 ...
	 * 控件值设置-操作
	 */
	public function setcontrval(){
		$formname=chr(rand(97, 122)).time();
		$func=$formname;
		$contr=$_REQUEST['control'];
		$par_name=$_REQUEST['par_name'];
		try {
			if (empty($contr)) {
				throw new Exception('请选择字段');
			}
			$formname='【'.$formname.'】';
			if(!empty($par_name)){
				$formname=$par_name;
			}
			$contrs=explode('|',$contr);
			if(count($contrs)<3){
				throw new Exception('字段信息错误');
			}
			$valedit_data=F('valedit_data');
			$formid=$valedit_data['formid'];//表单id
			if(empty($formid)){
			...
			$data=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
			$data[$formname]['type']="control";
			$data[$formname]['rules']=$contrs;
			$res['name']=$formname;
			$res['rules']=$contrs;
			$res['type']="control";
			if(!F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty,$data)){
				throw new Exception('设置失败，请关闭后重试！');
			}
			$this->ajaxReturn($res,'设置成功',1,'json');
		}catch(Exception $e){
			$this->ajaxReturn(null,$e->getMessage(),0,'json');
		}
	}
	 ...
	 * 接口值设置-页面
	 */
	public function apival(){
		$formname=$_REQUEST['myform'];//当前表单名和dialog的key
		$parentform=$_REQUEST['parentform'];//父级表单名
		$parentctr=$_REQUEST['parentctr'];//父级需要写入的控件name属性
		$valedit_data=F('valedit_data');
		$formid=$valedit_data['formid'];
		if(empty($formid)){
			throw new Exception('id错误，请重试');
		}
		if(empty($formname)||empty($parentform)||empty($parentctr)){
		...
		$datas=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
		$pars=$this->getpars($datas);

		$par_name=$_REQUEST['par_name'];
		if(!empty($par_name)){
			$rules=null;
			$rules['send_type']=$datas[$par_name]['send_type'];
			$rules['url']=$datas[$par_name]['url'];
			if(count($datas[$par_name]['param'])>0){
				foreach($datas[$par_name]['param'] as $k=>$v){
					$ite[]=implode("=",$v);
				}
				$rules['param']=implode("||",$ite);
			}
			$this->assign("api_rule",$rules);
		}
		$this->assign("par_name",$par_name);
	 ...
	 * 接口值设置-操作
	 */
	public function setapival(){
		$formname=chr(rand(97, 122)).time();
		$api_type=trim($_REQUEST['api_type']);
		$api_url=trim($_REQUEST['api_url']);
		$api_par=trim($_REQUEST['api_par']);
		$par_name=$_REQUEST['par_name'];
		try {
			$formname='【'.$formname.'】';
			if(!empty($par_name)){
			...
			if(empty($api_type)){
				throw new Exception('请选择接口类型');
			}
			if(empty($api_url)){
				throw new Exception('请填写接口地址');
			}
			$valedit_data=F('valedit_data');
			$formid=$valedit_data['formid'];//表单id
			if(empty($formid)){
			...
			$data=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
			$data[$formname]['type']="api";
			$data[$formname]['url']=$api_url;
			$data[$formname]['send_type']=$api_type;

			$res['name']=$formname;
			$res['url']=$api_url;
			$res['send_type']=$api_type;
			$res['type']="api";

			if(!empty($api_par)){
				$pars=null;
				$params=explode("||",$api_par);
				if(count($params)>0){
					foreach($params as $key=>$val){
						$pars[]=explode("=",$val);
					}
					if(count($pars)>0){
						$data[$formname]['param']=$pars;
					}
				}
				$res['par']=str_replace("||","&",$api_par);
			}
			if(!F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty,$data)){
				throw new Exception('设置失败，请关闭后重试！');
			}
	...
	public function spval(){
		...
		if(!empty($par_name)){
			$rules=null;
			$rules['spname']=$datas[$par_name]['spname'];
			if(count($datas[$par_name]['param'])>0){
				foreach($datas[$par_name]['param'] as $k=>$v){
					$ite[]=implode("=",$v);
				}
	 ...
	 * 存储过程值设置-操作
	 */
	public function setspval(){
		$formname=chr(rand(97, 122)).time();
		$sp_name=trim($_REQUEST['sp_name']);
		$sp_par=trim($_REQUEST['sp_par']);
		$par_name=$_REQUEST['par_name'];
		try {
			$formname='【'.$formname.'】';
			if(!empty($par_name)){
			...
			$data=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
			$data[$formname]['type']="sp";
			$data[$formname]['spname']=$sp_name;
...
//			}
	...
	public function attval(){
		...
		if(!empty($par_name)){
			$rules=null;
//			$rules['join']=$datas[$par_name]['join'];
//			$rules['filed']=$datas[$par_name]['filed'];
//			$rules['where']=$datas[$par_name]['where'];
			$rules['sql_str']=$datas[$par_name]['sql_str'];
			$this->assign("att_rule",$rules);
		}
		$this->assign("par_name",$par_name);
		$this->assign("pars",$pars);
	 ...
	 * 跨表取数设置-操作
	 */
//	public function setattval(){
//		$formname=chr(rand(97, 122)).time();
//		$join=trim($_REQUEST['atv_datas']);
//		$field=trim($_REQUEST['atv_rules']);
//		$where=trim($_REQUEST['atv_where']);
//		$par_name=$_REQUEST['par_name'];
//		try {
//			if (empty($field)) {
//				throw new Exception('请设置结果规则');
//			}
//			$formname='【'.$formname.'】';
//			if(!empty($par_name)){
//				$formname=$par_name;
...
//			$dmmap['isprimary']=1;
//			$dmmap['status']=1;
//			$formtable=M("mis_dynamic_database_mas")->where($dmmap)->field('tablename')->find();
//			if(!isset($formtable['tablename'])&&empty($formtable['tablename']))
//			{
//				throw new Exception('当前表数据不存在，请关闭后重试！');
//			}
//
//			$data=F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty);
//			$data[$formname]['type']='tables';
//			$data[$formname]['maintb']=$formtable['tablename'];
//			$data[$formname]['filed']=$field;
//
//
//			$res['name']=$formname;
//			$res['sql_str']="select {$field} from {$formtable['tablename']} ";
//			$res['type']="tables";
//
//			if($join){
//				$data[$formname]['join']=$join;
//				$res['sql_str'].=$join." ";
//			}
//			if($where){
//				$data[$formname]['where']=$where;
//				$res['sql_str'].= "where ".$where;
//			}
//			if(!F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty,$data)){
//				throw new Exception('设置失败，请关闭后重试！');
//			}
...
//	}
	...
	public function setattval(){
		$formname=chr(rand(97, 122)).time();
		$sql_str=trim($_REQUEST['atv_sql']);
		$par_name=$_REQUEST['par_name'];
		try {
			if (empty($sql_str)) {
				throw new Exception('请设置sql语句');
			}
			$formname='【'.$formname.'】';
			if(!empty($par_name)){
				$formname=$par_name;
	 ...
	 * 跨表取数数据控制
	 */
	public function attdatas(){
		$formname=$_REQUEST['myform'];//当前表单名和dialog的key
		$parentform=$_REQUEST['parentform'];//父级表单名
		$parentctr=$_REQUEST['parentctr'];//父级需要写入的控件name属性
		$parentctr1=$_REQUEST['parentctr1'];//父级需要写入的控件name属性
		if(empty($formname)||empty($parentform)||empty($parentctr)||empty($parentctr1)){
			throw new Exception('系统错误，请关闭窗口重试');
		}
		$valedit_data=F('valedit_data');
		$formid=$valedit_data['formid'];//表单id
		...
		$map['mis_dynamic_form_manage.status']=1;
		$map['mis_dynamic_database_mas.isprimary']=1;
//		$map['mis_dynamic_form_manage.id']=array('NEQ',$formid);
		$tables=$model->join('left join mis_dynamic_database_mas ON mis_dynamic_form_manage.id = mis_dynamic_database_mas.formid')->where($map)->field("mis_dynamic_form_manage.id,mis_dynamic_form_manage.actionname,mis_dynamic_form_manage.actiontitle,mis_dynamic_database_mas.tablename")->select();
		$list=array();
		$nowtable=null;
		foreach($tables as $key=>$val){
			if($val['id']!=$formid) {
				...
				$item['target'] = 'ajax';
				$item['rel'] = '';
				$item['title'] = $val['actiontitle'];
				$item['name'] = $val['actiontitle'];
				$item['tablename'] = $val['tablename'];
				$item['open'] = false;
				$item['isParent'] = false;
				$list[] = $item;
			}else{
		...
		$this->assign("decodelist",$list);
		$this->assign("list",json_encode($list));
		$this->assign("nowtable",$nowtable);
		...
		$this->assign('parentform',$parentform);
		$this->assign('parentctr',$parentctr);
		$this->assign('parentctr1',$parentctr1);
		$this->display();
	}
	/**
	 * 跨表取数数据控制-操作
	 */
	public function setatd(){
		try {
			$pform = $_REQUEST['pform'];
			if (empty($pform)) {
				throw new Exception('错误数据，请关闭后重试');
			}
			$objids = $_REQUEST['objid'];
			if (empty($objids) || count($objids) <= 0) {
				throw new Exception('请选择数据表');
			}
			$tbnames=$_REQUEST['tablename'];
			$rlinks = $_REQUEST['relationlink'];
			$rlconts = $_REQUEST['relationcont'];
			$joins=array();
			$joinids=array();
			foreach($objids as $key=>$val){
				$tbname=$tbnames[$val];
				$rlink=$rlinks[$val];
				$rlcont=trim($rlconts[$val]);
				if(empty($rlink)||empty($rlcont)||empty($tbname)){
					throw new Exception('错误数据，请关闭后重试');
				}
				$joinids[]=$val;
				$joins[]=$rlink." ".$tbname." ".$rlcont;
			}
			if(empty($joinids)){
				throw new Exception('错误数据，请关闭后重试');
			}
			...
			$data=array("ids"=>implode(",",$joinids),"joins"=>implode(" ",$joins));
			$this->ajaxReturn($data,'设置成功', 1, 'json');
		}catch(Exception $e){
			$this->ajaxReturn(null,$e->getMessage(), 0, 'json');
		}
	}
	 ...
	 * 跨表取数结果设置
	 */
	public function attrules(){
		$formname=$_REQUEST['myform'];//当前表单名和dialog的key
		$parentform=$_REQUEST['parentform'];//父级表单名
		$parentctr=$_REQUEST['parentctr'];//父级需要写入的控件name属性
		$parentctr1=$_REQUEST['parentctr1'];//父级需要写入的控件name属性
		$joinids=$_REQUEST['joinids'];//连接表id
		if(empty($formname)||empty($parentform)||empty($parentctr)||empty($parentctr1)){
			throw new Exception('系统错误，请关闭窗口重试');
		}
		$valedit_data=F('valedit_data');
		...
		$map['mis_dynamic_database_mas.isprimary']=1;
		if(!empty($joinids)) {
			$map['mis_dynamic_form_manage.id']=array('IN',$joinids.",".$formid);
		}else{
			$map['mis_dynamic_form_manage.id']=$formid;
		}
		$tables=$model->join('left join mis_dynamic_database_mas ON mis_dynamic_form_manage.id = mis_dynamic_database_mas.formid')->where($map)->field("mis_dynamic_form_manage.id,mis_dynamic_form_manage.actionname,mis_dynamic_form_manage.actiontitle,mis_dynamic_database_mas.tablename")->select();
		$list=array();
		$nowtable=null;
		...
		foreach($tables as $key=>$val){
			...
			$model=D("Dynamicconf");
			$resultlist=$model->getTableInfo($val['tablename']);
			if($resultlist){
				foreach ($resultlist as $tinfokey=>$tinfoval){
					$list[]=array(
						...
						'title'=>$tinfoval['COLUMN_COMMENT']."【{$val['actiontitle']}】【{$tinfoval['COLUMN_NAME']}】【{$tinfoval['DATA_TYPE']}】",
	...
	public function removepar(){
		...
		try {
			...
			$this->ajaxReturn($parname,'设置成功',1,'json');
		}catch(Exception $e){
			$this->ajaxReturn(null,$e->getMessage(), 0, 'json');
		}
	}
	...
	public function clearrules(){
		...
		try {
			...
			if(empty($cty)){
				throw new Exception('错误操作,请刷新重试');
			}
			if(!F('valedit_rules_'.$formid.'_'.$sort.'_'.$cty,array())) {
				throw new Exception('清除失败');
			}
			$this->ajaxReturn($sort, '清除成功', 1, 'json');
		}catch(Exception $e){
			$this->ajaxReturn(null, $e->getMessage(), 0, 'json');
		}
	}
	...
	public function getpars($datas){
		...
		foreach($datas as $key=>$val){
			...
			switch($val['type']){
				case 'control':
					$item['content']="《控件值》代码:{$key} 字段：".$val['rules'][2]."[{$val['rules'][1]}]";
					break;
				case 'api':
					$item['content']="《接口值》代码:{$key} 提交方式：".$val['send_type']." 地址:".$val['url'];
					if($val['param']){
						$item['content'].="?";
						foreach($val['param'] as $key=>$val){
							$item['content'].=implode('=',$val)."&";
						}
					}
                    $item['content']=rtrim($item['content'], '&');
					break;
				case 'sp':
					$item['content']="《存储过程》代码:{$key} 内容：".$val['spname'];
//					if($val['param']){
//						$item['content'].="(";
//						$par=null;
//						foreach($val['param'] as $key=>$val){
...
//					}
					break;
				case 'tables':
					$item['content']="《跨表取数》代码:{$key} sql语句：{$val['sql_str']}";

//					if(isset($val['join'])&&!empty($val['join'])){
//						$item['content'].=$val['join']." ";
//					}
//					if($val['where']){
//						$item['content'].= "where ".$val['where'];
//					}
					break;
				default:
					$item['content']="";
	 ...
	 *值更新公共js
	 */
	public function valeditcommonjs(){
		$jsstr=<<<EOF
/**
...
//var html = '<div class="g_layer_tips" style="display: none;"><p>'+ con +'</p><a href="javascript:;" onclick="$(this).parent().remove();" title="关闭">×</a></div>';
...
//tips.css({'top':'-'+(s_h+18)+ 'px'}).show();
	 ...
	 * @return string  
	 * @author 黎明刚
	 * @date 2017年9月12日 下午10:44:16 
	 * @throws
	 */
	public function valViewJs($formid){
		try {
			$model = D('MisValueEditRules');
			$res = $model->getpptyrulestree($formid);
			$jstext = "";
			if ($res) {
				$jstext=<<<EOF
...
EOF;
				...
				foreach ($res as $key => $val) {
					...
					foreach ($val['rules'] as $k => $v) {
						//逻辑规则参数js
						$ljr_param_js = $this->getrulesvar(json_decode($v['lj_rules'], true));
						//替换逻辑规则参数
						$ljcon = html_entity_decode($v['lj_content']);
						preg_match_all('/【([a-z0-9]{11,11})】/', $ljcon, $matches);
						foreach ($matches[0] as $mk => $mv) {
							$ljcon = str_replace($mv, $matches[1][$mk], $ljcon);
						}
						//操作对象
						$opobj = json_decode($v['act_obj']);
						if(empty($opobj)){
							throw new Exception('js生成失败，操作对象不存在，id:'.$v['id']);
						}
						//操作类型
						$optype=$v['act_type'];
						if(empty($optype)){
							throw new Exception('js生成失败，操作类型不存在，id:'.$v['id']);
						}
//						print_r('=======================================================================');
						//操作结果
						$isact=$v['is_act'];
						//判断是否需要结果规则
						$jgr_param_js="";
						if(in_array($optype,array(3,4,5))){
							$jgr_param_js.=$this->opobjvaljs($opobj[0],$opobj[1],$optype,$isact);
						}else{
							//结果规则参数
							$jgr_param_js=$this->getrulesvar(json_decode($v['jg_rules'],true));
							//结果规则内容
							$jgcon=html_entity_decode($v['jg_content']);
							//替换结果规则参数
							preg_match_all('/【([a-z0-9]{11,11})】/', $jgcon, $matche);
							foreach ($matche[0] as $mk1 => $mv1) {
								$jgcon = str_replace($mv1, $matche[1][$mk1], $jgcon);
							}
							$jgr_param_js.=$this->opobjvaljs($opobj[0],$opobj[1],$optype,$jgcon);
						}
						if ($ljcon) {
							$ljr_param_js .= <<<EOF
...
EOF;
	 ...
	 * 获取表单值更新js
	 */
	public function valeditjs($formid){
		try {
			$model = D('MisValueEditRules');
			$res = $model->getpptyrulestree($formid);
			$jstext = "";
...
EOF;
					...
					if($val['edit_type']==1){
						$jstext.=$this->objchangejs($val['category'],$val['fieldname']);
						$jstext.=<<<EOF
...
EOF;
	 ...
	 * @param $cname
	 */
	public function objchangejs($ctype,$cname){
		$changejs="";
		if($ctype=="text") {
			$changejs=<<<EOF

		$("input[name='{$cname}']",box).on('change',function(){
EOF;
		}
		if($ctype=="select") {
			$changejs=<<<EOF

		$("select[name='{$cname}']" , box).on('change' ,function(){
EOF;
		}
		if($ctype=="radio") {
			$changejs=<<<EOF

		$("input[name='{$cname}']").on('change',function(){
EOF;
		}
		if($ctype=="checkbox") {
			$changejs=<<<EOF
...
EOF;
		...
		$("textarea[name='{$cname}']").on('change',function(){
EOF;
		}
		return $changejs;
	}
	 ...
	 * 根据参数操作结果值 生成js
	 * @param $ctype 控件类型
	 * @param $cname 控件name属性
	 * @param $gtype 操作类型
	 * @param $val 操作值
	 */
	public function opobjvaljs($ctype,$cname,$gtype,$val){
		$jsstr="";
		$jsval="";
		switch($gtype){
			//赋值
...
EOF;
			...
			case 5:
				if($val==1) {
					$jsval = ".addClass('required');";
				}else{
					$jsval = ".removeClass('required')";
				}
				break;
			case 6:
				$jsval=".prop('src',{$val});";
...
EOF;
			if($gtype==1){
				...
				redio_chk($("input[name='{$cname}']",box),{$val});
EOF;
			}
			if($gtype==2){
				$jsstr=$jsval;
...
EOF;
			if($gtype==1){
				...
				checkbox_chk($("input[name='{$cname}[]']",box),{$val});
EOF;
			}
			if($gtype==2){
				$jsstr=$jsval;
...
EOF;
	 ...
	 * @return string
	 */
	public function getrulesvar($rules){
		$str="";
		if($rules&&count($rules)>0){
			foreach($rules as $key=>$val){//规则参数循环
				$var=str_replace("【","var ",$key);
				$var=str_replace("】","",$var);
				if($val['type']=='control'){
					switch($val['rules'][0]){
						case 'text':
...
EOF;
				...
				{$var}=$("select[name='{$val['rules'][1]}']",box).val();
EOF;
							break;
						case 'radio':
							$str.=<<<EOF

				{$var}=getradioval('{$val['rules'][1]}');
EOF;
							break;
//						case 'checkbox':
//							$kjname="select[name='{$val['rules'][1]}']";
//							break;
//						case 'textarea':
//							$kjname="textarea[name='{$val['rules'][1]}']";
//							break;
						default:
							break;
					}
				...
				if($val['type']=='sp'){
					$option="{";
					$option.="gtype:'sp'";
					$option.=",spname:'".html_entity_decode($val['spname'])."'";
                    preg_match_all('/【([a-z0-9]{11,11})】/', $option, $matches);
                    if($matches[0]){
                        foreach($matches[0] as $k=>$v){
                            $option=str_replace($v,"'+{$matches[1][$k]}+'",$option);
                        }
                    }
//					if($val['param']){
//						$option.=",param:{";
...
//								$option.=",";
//							}
//							if(strstr($v[1],"【")&&strstr($v[1],"】")){
//								$v[1]=str_replace("【","",$v[1]);
//								$v[1]=str_replace("】","",$v[1]);
//								$option.=$v[0].":".$v[1];
//							}else{
//								$option.=$v[0].":'".html_entity_decode($v[1])."'";
//							}
//						}
//						$option.="}";
//					}
...
EOF;
				...
				if($val['type']=='api'){
					$option="{";
					$option.="gtype:'api'";
					$option.=",url:'".html_entity_decode($val['url'])."'";
					$option.=",stype:'".$val['send_type']."'";
					if($val['param']){
						$option.=",param:{";
						foreach($val['param'] as $k=>$v){
							if($k>0){
...
EOF;
				...
				if($val['type']=='tables'){
					$option="{";
					$option.="gtype:'tables'";
					$option.=",sql_str:'".html_entity_decode($val['sql_str'])."'";
//					$option.=",maintb:'".html_entity_decode($val['maintb'])."'";
//					$option.=",filed:'".html_entity_decode($val['filed'])."'";
//					$option.=",join:'".html_entity_decode($val['join'])."'";
//					$where=html_entity_decode($val['where']);
					preg_match_all('/【([a-z0-9]{11,11})】/', $option, $matches);
					if($matches[0]){
						foreach($matches[0] as $k=>$v){
                            $option=str_replace($v,"'+{$matches[1][$k]}+'",$option);
