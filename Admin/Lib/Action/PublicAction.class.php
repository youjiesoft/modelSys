...
//		$adminArr = $userDao->where("status = 1")->getField("id,account");
	...
	public function signin() {
		...
		//保存信息用于后面的单点登陆取值，单点登陆只支持中文登陆
		$userModel=M('user');
		if($_POST['account']=="admin"){
            $_SESSION['username']=$_POST['account'];
		}else{
            if(preg_match("/[A-Za-z]/",$_POST['account'])){
                $zhdata['account']=$_POST['account'];
                $zhname=$userModel->field('zhname')->where($zhdata)->find();
                $_SESSION['username']=$zhname['zhname'];
            }else{
                $_SESSION['username']=$_POST['account'];
            }
		}
...
//             }
        ...
        echo json_encode(array_merge($url,$userAuditlist));
	}
