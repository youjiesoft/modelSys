...
class CommonAction extends CommonExtendAction {
	...
	function _initialize() {
		if(ACTION_NAME!="curl_export_word" && ACTION_NAME!="misFileManageDownload"){
...
// 			$voList = $model->query($sql);
	 ...
	 * @throws ThinkExecption
	 +----------------------------------------------------------
	 */
	protected function _list($name, $map, $sortBy = '', $asc = false,$group='',$echoSql='',$sortstr='') {
		import ( '@.ORG.Browse' );
		//map附加
		//提醒条件
...
// 			echo $model->getLastSql();
	...
	function insert($isrelation=false) {
		$name=$this->getActionName();
		logs($_POST , $name.'_add_'.date('Y-m-d-H' , time()) ,'',__CLASS__,__FUNCTION__,__METHOD__);
		...
		if ($list!==false) {
			...
			if ($_POST['auditFlowTuiTablename'] && $_POST['auditFlowTuiTableid'] && $_POST['auditZhuLicModel'] && $_POST['auditZhuLicId']) {
				...
				$process_relation_formDao->where($map)->setField("isaudittableid",$list);
			}
			//上传附件信息
			...
			if (method_exists($module2,"_over_insert")) {
				//这里用来存储临时缓存文件
// 				$updateBackup=self::setOldDataToCache($model,$name,$list,'insert');
...
// 					$lastID = $model->query('SELECT LAST_INSERT_ID() as returnval');
// 					$list = $lastID[0]['returnval'];
					//echo 'sssssssssssssssssssssssssssssssss'.$list;
					return $list;
				}
...
// 	}
	...
	function view($num=1){
		...
		if($result){
			$remindModel->where($remindMap)->setField("isread",1);
		}
...
// 		dump($vo);
	...
	public function _before_overProcess($id) {
		//流程项目任务
		$data = D($this->getActionName())->find($id);
		...
		if(1==1){
			//找到原始流程的对象及ID
			$map = array();
			$map['isauditmodel']=$this->getActionName();
			...
			*/
			...
			try{
				foreach ($cacheFormData as $key => $val){
					...
					if($tempAction == 'MisAutoEqe'){
					    throw new NullDataExcetion($tempAction);
					}
...
// 							}
				...
				if(!$isret){
					...
					$this->error("批量单据操作失败，已回滚 action【{$tempAction}】" );
				...
				$model->rollback();
				// getMessage()
				$this->error( $e->getMessage().' _1处理错误，数据回滚！' );
			}
		}else{
		// 旧版套表	
			$model = new Model();
			...
			try{
				// 取得本次操作的所有action列表。
				$actionnamelist = $_POST['__actionnamelist__'];
				$findCacheMap['uid'] = $_SESSION[C('USER_AUTH_KEY')];
				$findCacheMap['keys'] = $actionnamelist;
				$findCacheMap['ip'] = getip();
				
				// 操作批次标识
				$oprateTag = $_POST['__main__'];
				
				if($actionnamelist && $oprateTag){
					if($_POST['__actionlistend__'] == 'end'){
						$retArr=array();
						// 开始处理数据
						// 获取缓存数据，按写入人与操作批次
						$cacheModel = M('mis_system_form_cache');
						$cacheFormData = $cacheModel->where($findCacheMap)->select();
						// 主动清除缓存数据。
						$model->startTrans();
						$cacheModel->where($findCacheMap)->delete();
						$cacheModel->commit();
						...
						try{
							foreach ($cacheFormData as $key => $val){
								$_POST = unserialize($val['cache']);
								unset($tempAction);
								unset($tempObj);
								$tempAction = $val['actionname'];
								$tempObj = A($tempAction);
								C("TOKEN_ON",false);
								$isret="";
								switch ( $val['oprate'] ){
							...
							if(!$isret){
								...
								//if($isStartProcess){
									//$this->success('流程已启动！' , '' , array( 'id'=>$retArr[$oprateTag] ,'bindid'=>$bindid,'isprocess'=>$isStartProcess) );
							//	}else{
									$this->success('批量据操作功能' , '' , array( 'id'=>$retArr[$oprateTag] ,'bindid'=>$bindid,'isprocess'=>$isStartProcess ) );
							//	}
							}
							...
							$cacheModel->commit();
							// getMessage()
							$this->error( $e->getMessage() .' _处理错误，数据回滚！' );
						}
					}else{
						$actionnamelistArr = explode(',', $actionnamelist);
						$selfAction = $_POST['__selfaction__'];
						$selfOprate = $_POST['__selfoprate__'];
						if(in_array($selfAction, $actionnamelistArr)){
							$cacheModel = M('mis_system_form_cache');
							$cacheData['oprate']	= $selfOprate;
							$cacheData['uid']	= $_SESSION[C('USER_AUTH_KEY')];
							$cacheData['actionname']	=	MODULE_NAME;
							$cacheData['ip']	=	getip();
							$cacheData['createtime']	=	time();
							$cacheData['cache']	=	serialize($_POST);
							$cacheData['keys']	=	$actionnamelist;
							
							
							$cacheModel->data($cacheData)->add();
							$cacheModel->commit();
						}
					}
					//$this->update(true);
		...
		if($vo){
			...
			if(getFieldBy($name, "bindaname", "inbindaname", "MisAutoBind","typeid",2)){
				...
				if(!$iszctpl){//组合表单
					...
					$MisAutoBindSettableList = $MisAutoBindModel->where($bindMap)->order("inbindsort asc")->getField("id,inbindaname,bindtype,bindaname,inbindtitle,inbindsort,formshowtype");
				}else{
					$bindid=$vo['id'];
					// 查询符合条件的表单
	...
	function area_info($inserid , $modelName=''){
		if($_POST['areainfotag']){
			...
			foreach ($dataArr as $key=>$val){
				...
				$modelName=$modelName ? $modelName : $tablename;
				// 实例地区信息表
				$model = D('MisAddressRecord');
				if(C('AREA_TYPE')==1){
					$modelName = $modelName ? $modelName : $this->getActionName();
				}elseif(C('AREA_TYPE')==2){
					$modelName = $modelName ? $modelName : (D($this->getActionName())->getTableName());
				}
				$data = '';
...
AND STATUS=1";
		...
		if ($lookupDetail ['condition']) {
			$newconditions = $lookupDetail ['condition'];
		}
		if ($appendCondition) {
		...
		if($lookupKey=='2ba96fc5ab7bef0ac9a4d3f5d72bb1f0'){//2ba96fc5ab7bef0ac9a4d3f5d72bb1f0 b6fe179d38c8034cee79b5b09dd6b1e7
...
// 			$this->assign("viewname",$_REQUEST['viewname']);
	...
	function lookupson(){
		...
		$userid = $_SESSION[C('USER_AUTH_KEY')];		
		$this->assign('modelname',$_POST['model']);
...
// 		}
		...
		$filepath=DConfig_PATH . "/Models/".$_POST['model']."/{$datename}.inc.php";
		...
		foreach($list['list'] as $k1=>$v1){
			...
			foreach($funcv1 as $k3=>$v3){			
				if($dtfuncfields&&in_array($k3,$dtfuncfields)&&count($dtfunc[$k3]['funcname'])>0){
					$funcv1[$k3] = getConfigFunction($v3,$dtfunc[$k3]['funcname'][0],$dtfunc[$k3]['funcdata'][0],$funcv1);
				}
			}
...
// 			$newlist = json_decode(html_entity_decode($lookupjson),true);
	...
	function lookupinsert(){
		...
		$temp = explode($_POST['list'][0],'|');
		$data['mainid'] = $temp[2];//$_POST['masid'];
...
// 							print_r($orderSql);exit;
	...
	public function getMiniIndexLoad(){
		...
		if($fieldtype||$_REQUEST['bindrdid']){
			...
			$datarommasid=$MisAutoBindSettableVo['dataroamid'];
...
EOF;
		...
		foreach ($retTableInfoArr as $rkey=>$rval){
			...
			if($rval['id']){
				...
				if($rval['typeid']==2){
					$tablesHtml.="<li {$selected}><a id=\"{$rval['actionname']}_#tagitem#\" class=\"j-ajax\" rel=\"{$main}{$rval['actionname']}_edit\" href=\"".__APP__."/{$rval['actionname']}/miniindex/bindtype/{$rval['bindtype']}/bindaname/{$action}/fieldtype/{$rval['fieldtype']}/{$rval['fieldtype']}/{$val}{$mainBindActionCondition}\">";
				}else{
					$tablesHtml.="<li {$selected}><a id=\"{$rval['actionname']}_#tagitem#\" class=\"j-ajax\" rel=\"{$main}{$rval['actionname']}_edit\" href=\"".__APP__."/{$rval['actionname']}/miniindex/bindtype/{$rval['bindtype']}/bindaname/{$action}/bindid/{$val}{$mainBindActionCondition}\">";
				}			
				$tablesHtml.="<span>".$inbindtitle."</span>";
				$tablesHtml.="</a></li>";
				$footerhtml.="<div id=\"{$main}{$rval['actionname']}_edit\" style=\"{$block}\"></div>";
...
// 		M()->execute("DELETE FROM  ".$randtable." WHERE randnum='".$randnum."' and createid='".$_SESSION[C('USER_AUTH_KEY')]."';");
	...
	public function SaveVersionWord($id,$name,$isrelation=false,$return=false){
		...
		$where['tableid'] = $id;
		$where['flowtype'] = array(' in ',array(2,3));
		$where['catgory'] = 0;//普通流程节点，非变更
		$relation_formlist = $process_relation_formDao->where($where)->field("id,relationid,flowid,name")->select();
		if(count($relation_formlist)>0){
			foreach ($relation_formlist as $rk=>$rv){
				...
				$list[substr(md5($rv['flowid']."HS_name"),0,8)] = array('name'=>substr(md5($rv['flowid']."HS_name"),0,8),'showname'=>'','original'=>'','value'=>$rv['name']);
				$list[substr(md5($rv['flowid']."HS_doinfo"),0,8)] = array('name'=>substr(md5($rv['flowid']."HS_doinfo"),0,8),'showname'=>'','original'=>'','value'=>$Historyvo['doinfo']);
				$list[substr(md5($rv['flowid']."HS_dotime"),0,8)] = array('name'=>substr(md5($rv['flowid']."HS_dotime"),0,8),'showname'=>'','original'=>'','value'=>transTime($Historyvo['dotime'],'Y/m/d H:i'));
				$list[substr(md5($rv['flowid']."HS_userid"),0,8)] = array('name'=>substr(md5($rv['flowid']."HS_userid"),0,8),'showname'=>'','original'=>'','value'=>getFieldBy($Historyvo['userid'], 'id', 'name', 'user'));
			}
		}
		if($return){
			return $list;
			...
			//分表存储list的数据
			$randtable="mis_system_template_saveword_list".(fmod($id,10)+1);//通过当前数据id除以分表数量的余数来确定缓存哪张表,被除数分表开关
			$cacheTable=M()->execute("CREATE TABLE if not exists `".$randtable."` (
					  `id` int(11) NOT NULL AUTO_INCREMENT,
					  `modelname` varchar(100) DEFAULT NULL COMMENT '模型名',
					  `modelid` int(11) DEFAULT NULL COMMENT '对应id',
					  `savewordid` int(11) DEFAULT NULL COMMENT 'saveword表id',
					  `listdata` longtext COMMENT 'list数据',
			...
			if($cacheTable===false){
				if(!$isrelation){
					$this->error("建立数据缓存表错误");
				}else{
					$msg = "版本控制建立数据缓存表错误 [sql error]".$savewordModel->getError ().'[action]'.$name;
					throw new NullPointExcetion($msg);
				}
			}
			$listModel=M($randtable);
			$listData['listdata']=serialize($list);
			$listData['modelname']=$name;
			$listData['modelid']=$id;
			$listData['savewordid']=$query;
			$listData['createid']=$_SESSION[C('USER_AUTH_KEY')];
			$listResult=$listModel->add($listData);
			if($listResult==false){
				if(!$isrelation){
					$this->error("版本控制数据保存失败");
				}else{
					$msg = "版本控制数据保存失败 [sql error]".$savewordModel->getError ().'[action]'.$name;
					throw new NullPointExcetion($msg);
				}
			}
		}
	...
	public function export_word_one(){
		...
		//获取文件数据ID，判定存入的表
		$id=$_REQUEST['id'];logs("===========>".$id,"GMLX2X2");
		$modelname=$this->getActionName();
		//获取数据
		...
		if($temp){
			foreach($temp as $k => $v){
				if($v["rules"]){
					...
					$map="";
					$map['_string']=$v["rules"];
					$findList = $model->where($map)->find();logs($model->getlastSql(),"GMLX1X1");
					if($findList){
						$findList = $v;
						break;
					}
