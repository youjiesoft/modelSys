	...
	public function misimportexceladd(){
		...
		if( $this->escapeStr($_GET['download']) >0 ){
            ...
            for($i=2;$i<=65;$i++){
                foreach($_exportData as $k => $v){
                    $colum = PHPExcel_Cell::stringFromColumnIndex($k);
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($colum.$i, "");
                }
            }
...
// 			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', "2014");
    ...
    public function misimportexcelinsert(){
        ...
        if($step == 2 ){
            ...
            if(empty($datatablemodel)){
                ...
                if( ! empty($dataList['data'])){
                    ...
                    foreach($dataList['data'] as $k => $v){
                        if($k!=1){
                            $temp = array();
                            foreach($v as $kk => $vv){
                                if(empty($title[PHPExcel_Cell::columnIndexFromString($kk)-1])){
                                    $a=$a+1;
                                    $temp[$a]=$vv;
                                }else{
                ...
                if($dataguodu){
                    $datadaihuizhi=M('mis_auto_zaprt_sub_zhibiaonarong')->field('zhibiaomingchen,zhibiaobianma,masid')->select();
                    foreach ($datadaihuizhi as $key1=>$val1){
                        $datadaihuizong[$key1]['narong']=$val1['zhibiaomingchen'].'_'.$val1['zhibiaobianma'];
                        $datadaihuizong[$key1]['masid']=$val1['masid'];
                    }
                    foreach ($datadaihuizong as $key1=>$val1){
                        $datadaihuizong1[$val1['masid']][]=$val1['narong'];
                    }
                    foreach($dataguodu as $key1=>$val){
                        foreach ($datadaihuizong1 as $key2=>$val2){
                            foreach ($val2 as $key3=>$val3){
                                $data[$key1][$val3]=$val[$key3];
                            }
                        }
                    }
                }
                ...
                //删除数组表格中为空的元素行
                foreach($ajaxData['sheet_list']  as $key=>$val){
                    $num =0;
                    foreach($val as $v){
                        if(!$v){
                            $num += 0;
                    ...
                    if($num==0){
                        array_splice($ajaxData['sheet_list'],$key);
                    }
                }
                $this->ajaxReturn($ajaxData);
            }
     ...
     * 导入excel功能
     */
    public function misimportonlyinsert(){
        $this_model=$_REQUEST['model'];
        import ( '@.ORG.UploadFile');
        if($_FILES){
            $upload = new UploadFile(); // 实例化上传类
            $upload->savePath = UPLOAD_PATH."/tempImportExcel/";//C('savePath'); // 上传目录
        ...
        if($filetype=="xls" || $filetype=="xlsx") {
            ...
            for($i=1;$i<=$highestRow;$i++){
                for($j='A';$j<=$highestColumn;$j++){
                    $data[] = $this->trimall($objPHPExcel->getActiveSheet()->getCell($j.$i)->getValue());
                }
            }
        }
        $model = new Model();
        //查询表单导入对应关系表内的内嵌表数据用来组装数组
        $sql= "select ziduanming,xlsxingziduanming from mis_auto_whmji_sub_ziduanguanxi where masid=(select id from mis_auto_whmji where excelbiaoqianming='$sheetname')";
        $chat = $model->query($sql);
        $result = array();
        $a=1;
        foreach($chat as $key=>$val){
            foreach($data as $va){
                //取相同字段的数据
                if($val['xlsxingziduanming'] == $va){
                    $a=$a+1;
                    $index = array_search($va,$data);
                    $str = $val['ziduanming'];
                    $st = preg_replace('/\d+/','',$str);
                    if(!is_numeric($data[$index+$a])){
                        $result[$st]= $data[$index+1];
                        $a=1;
                        continue;
                    }
                    $result[$st]= $data[$index+$a];
                    if($a==3){
                        $a=1;
                    }
                }
