...
// 		if ($_SESSION["a"] != 1) $map['status']=array("egt",0);
    ...
    protected function _viewlist($name, $map, $sortBy = '', $asc = false, $group = '', $echoSql = '', $sortstr = '', $renyuan = '')
    {
        $model = D($name);
        //视图信息
        $sysviewmap = array();
...
// 			$voList = $model->query($sql);
            if ($renyuan == 1) {
                $voList = $model->where($map)->order('mis_hr_personnel_person_info.sort asc,id asc')->group($viewInfo['groupcondition'])->limit($p->firstRow . ',' . $p->listRows)->query($viewInfo['spellwheresql'], true);
            } else {
...
// 			echo $htmls;
        ...
        if ($count > 0) {
             ...
             *  查询该表 是否存在地址信息
             */
            $areamap['actionname'] = $this->getActionName();
            $infoarray = M('mis_dynamic_form_areainfo')->where($areamap)->select();

            foreach ($voList as $vkey => $val) {
                //获取所有的字段名
...
EOF;
            ...
            foreach ($voList as $key => $val) {
                if ($val['zuojia'] == '0.00') {
                    $voList[$key]['zuojia'] = NULL;
                }
                if ($val['upload75'] == 'upload75') {
                    $voList[$key]['upload75'] = NULL;
                }
            }
            $this->assign('list', $voList);
//			dump($voList);
//			exit;
    ...
    {
        $data = $_POST;
        $shuzuyi = explode('_', $data['biaoqianzui']);
        unset($shuzuyi[count($shuzuyi) - 1]);
        $zifuyi = implode('_', $shuzuyi);
        $zifuer = $zifuyi . '_';
        $zhibiaomingchen = array();
        $xiugaiyici = array();
        $xiugaierci = array();
        $xiugaisanci = array();
        $xiugaisici = array();
        $Tepmprowidgrid = "";
        $twodimensional = array();  //存储二维表字段名
        $postok = array();;

        foreach ($data['istwodimensional'] as $key => $twodimensionalvalue) //找出是表格的数组
        {
            if ($twodimensionalvalue == 1) {
                $twodimensional[] = $key;  //得到二维表字段名
            }
        }
        $erweibiaonu = count($twodimensional);
        if ($erweibiaonu) {
            $a = 0;
            $a1 = 0;
            $a2 = 0;
            foreach ($data['datatable'] as $keyone => $valueone) //找出是表格的数组
            {
                //$Tepmprowidgrid=
                foreach ($valueone as $key => $val) //得到了一行记录
                {
                    if (in_array($key, $twodimensional))  //是否在二维表字段名
                    {
                        foreach ($val as $keyend => $valend) //得到了具体的字段值
                        {

                            $zhibiaomingchen[$key][$a][$keyend] = $valend;

                            if (isset($data['datatable'][$a][$key])) {
                                unset($data['datatable'][$a][$key]);
                                $a1 = $a1 + 1;
                            }
                            if ($keyend == 'id') {
                                $a = $a + 1;
                        ...
                        $xiugaisanci[$keyone] = $data['datatable'][$keyone];
                    }
                }
            }
//        for($i=0;$)
            ...
            $sh = '';
            $nu = 0;
            foreach ($zhibiaomingchen as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    if ($sh != $key1) {
                        $sh = $key1;
                        $nu = 0;
                        $beiyong[$key1][$nu] = $val2;
                        $beiyong[$key1][$nu]['rowcontens'] = $nu + 1;
                        $nu = $nu + 1;
                    } else {
                        $beiyong[$key1][$nu] = $val2;
                        $beiyong[$key1][$nu]['rowcontens'] = $nu + 1;
            ...
            $beiyong1 = array();
            foreach ($beiyong as $key1 => $val1) {
                $b = 0;
                foreach ($val1 as $key2 => $val2) {
                    $nu = $val2['rowcontens'];
                    foreach ($val2 as $key3 => $val3) {
                        if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $key3)) {
                            $k2 = explode('_', $key3);
                            $beiyong1[$key1][$b]['zhibiaomingchen'] = $k2[0];
                            $beiyong1[$key1][$b]['zhibiaobianma'] = $k2[1];
                            $beiyong1[$key1][$b]['jine'] = $val3;
                            $beiyong1[$key1][$b]['rowcontens'] = $beiyong[$key1][$key2]['rowcontens'];
                            unset($beiyong[$key1][$key2][$key3]);
                            $b = $b + 1;
                            $chazhaoxx01[$key1] = $b;
                            $chazhaoxx02[$key1] = $beiyong[$key1][$key2]['rowcontens'];
                        } else {
                            continue;
                        }
                    }
...
//        var_dump($beiyong1);
//        exit;
            foreach ($chazhaoxx01 as $key1 => $val1) {
                $chazhaoxx[$key1] = $val1 / $chazhaoxx02[$key1];
            }
            $c = 0;
            $d = 0;
            foreach ($beiyong as $key1 => $val) {
                foreach ($val as $key2 => $val2) {
                    $beiyong2[$key1][($key2 + 1) * $chazhaoxx[$key1]] = $val2;
                }

            }
            $ss = 0;
            foreach ($beiyong2 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    for ($ss; $ss < $key2; $ss++) {
                        $beiyong3[$key1][$ss] = $val2;
                    }
                    $ss = $key2;
                }
                $ss = 0;
            }
            foreach ($beiyong3 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if ($key3 != 'id' && $key3 != 'jine') {
                            $beiyong1[$key1][$key2][$key3] = $val3;
                        }
                    }
                }
            }
            foreach ($beiyong1 as $key => $val) {
                $zi = 0;
                foreach ($val as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $zhibiaomingchen1[$zi][$key] = $val1;
                    }
                    $zi = $zi + 1;
                }
            }
            $g = 0;
//        foreach($zhibiaomingchen1 as $key1=>$val1){
//            foreach($val1 as $key2 => $val2){
//                $xiugaiyici[$key1][$g]=$val2;
//                $g=$g+1;
//            }
//        }

            $xiugaierci = $zhibiaomingchen1;

            $f = 0;
            foreach ($xiugaiyici as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $xiugaierci[$f][$key1] = $val2;
                    $f = $f + 1;
                }
            }
            $data['into_table_serial'] = count($xiugaierci) - 1;
//        for($i=0;$i<$a1;$i++){
//            $data['datatable']=array_merge($data['datatable'],$xiugaierci);
//        }
//        foreach(){
//
//        }
            foreach ($xiugaisanci as $key1 => $val1) {
                $b = 0;
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $key3)) {
                            unset($xiugaisanci[$key1][$key2]);
                        } else {
                            continue;
                        }
                    }
            ...
            if (count($xiugaisanci) > count($xiugaierci)) {
                for ($i = 0; $i <= count($xiugaisanci); $i++) {
                    if (empty($xiugaierci[$i])) {
                        $data['datatable'][$i] = $xiugaisanci[$i];
                    } else {
                        $data['datatable'][$i] = array_merge($xiugaisanci[$i], $xiugaierci[$i]);
                    }
                }
            } else {
                for ($i = 0; $i <= count($xiugaierci); $i++) {
                    if (empty($xiugaisanci[$i])) {
                        $data['datatable'][$i] = $xiugaierci[$i];
                    } else {
                        $data['datatable'][$i] = array_merge($xiugaierci[$i], $xiugaisanci[$i]);
                    }
                }
            }
            foreach ($data['datatable'] as $key1 => $val1) {
                $b = 0;
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $key3)) {
                            unset($data['datatable'][$key1][$key2]);
                        } else {
                            continue;
                        }
                    }
...
// 								$list = $lastID[0]['returnval'];
            ...
            if (!$isrelation) {
                $this->error($model->getDberror() . L('_ERROR_'));
            } else {
                ...
                if ($v['status'] != -1) {
                    ...
                    if ($v['models']) {
						...
						*/
                        ...
                        //处理无法正常打开项目评审汇总单
                        if ($models == 'MisAutoUid' || $models == 'MisAutoJwf') {
                            $url .= "/pingshenhuileixing/5";
                        }
                        //此处默认方法session权限，表单内部的url权限问题
...
// 		print_r($vo);
    ...
    function edit($num = 1)
    {
        ...
        if ($name == 'MisAutoUsf') {
            $vo = M('mis_auto_izvbe')->where($map)->find();
        }
        //echo $model->getLastSql();
        $retSql = <<<EOF
...
//        dump($areaMap);exit;
        foreach ($areaMap as $key => $val) {
            $areainfolist[$val['fieldname']] = array_merge($val, $areainfoarry[$val['fieldname']]);
        }
...
//		$module->_after_edit();
    ...
    public function _before_overProcess($id)
    {
        ...
        if (1 == 1) {
            ...
            logs(M('process_relation_children')->getlastSql(), "gml11-20-1");
            logs($obj, "gml11-20-1");
            if (!$obj) {
                //为了匹配以前的老数据
                //找到原始流程的对象及ID
                ...
                logs(M('process_relation_form')->getlastSql(), "gml11-20-2");
                if ($obj) {
    ...
    function updateControll()
    {
        ...
        $oldrequest = $_REQUEST;
        $originalPost = $_POST;
        $pdliucheng = $originalPost['__apply__form__'][0];
        ...
        foreach ($pdlc as $key1 => $val1) {
            if ($val1['shifushengxiao'] == '是') {
                if ($val1['kongzhishidian'] == '提交前' && !empty($pdliucheng['doinfo'])) {
                    ...
                    if ($datalc[0]['@result'] == 1) {
                        $this->error($datalc[0]['@msg']);
                    } elseif ($datalc[0]['@result'] == -1) {
                        $this->error('系统错误');
                    }
                } elseif ($val1['kongzhishidian'] == '保存前' && empty($pdliucheng['doinfo'])) {
                    if (strpos($val1['ziduanming'], '&')) {
                        $hb = explode("&", $val1['ziduanming']);
                        foreach ($hb as $key2 => $val2) {
                            $liuc = $liuc . $pdliucheng[$val2];
        ...
        if ($originalPost['__apply__form__'] && $originalPost['__actionlistend__'] == 'end') {
            ...
            //下一阶段审批人查询条件生成
            $doinfo = $cacheFormData[0]['doinfo'];
            $tableidp = $cacheFormData[0]['id'];
            $tablenamep = $cacheFormData[0]['__main__'];
            //	遍历真实提交表单数据
                    ...
                    switch ($selfOprate) {
                        ...
                        case 'edit':
                            if ($_POST['__main__'] == $_POST['__selfaction__']) {
                                ...
                                } elseif (10 == $_POST['__startprocessStatus__']) {
                                    //审核
                                    $isStartProcess = $isret = $tempObj->auditProcess(true);
                                } else {
                                    // 是主表
...
// 							}
                ...
                if (!$isret) {
                    ...
                    unset($oldrequest);
                    //弹出下一阶段审批人
                    $process_relation_formmodel = M("process_relation_form");
                    $person_info = M("mis_hr_personnel_person_info");
                    $resultperson = $person_info->field("name,userid")->select();
                    $where = array();
                    $where['tableid'] = $tableidp;
                    $where['tablename'] = $tablenamep;
                    $where['doing'] = 1;
                    $where['flowtype'] = array('gt', 1);
                    $where['auditstate'] = array('eq', 0);
                    $resultform = $process_relation_formmodel->field('curaudituser')->where($where)->select();
                    $rename = array();
                    foreach ($resultform as $key => $val) {
                        foreach ($resultperson as $ke => $va) {
                            if ($val['curaudituser'] == $va['userid']) {
                                array_push($rename, $va['name']);
                            }
                        }
                    }
                    if ($doinfo) {
                        $this->success('批量单据保存成功,下一阶段处理人是->' . $rename[0], '', array('id' => $retArr[$oprateTag], 'bindid' => $bindid, 'isprocess' => $isStartProcess));
                    } else {
                        $this->success('批量单据保存成功了', '', array('id' => $retArr[$oprateTag], 'bindid' => $bindid, 'isprocess' => $isStartProcess));
                    }
                    //	}
                }
    ...
    function update($isrelation = false)
    {
        ...
        if (count($twodimensional)) {
            //此处修改
            if (empty($data['#hide#datatable']) && !empty($twodimensional)) {
                $pdmsid = $data['masid'];
                foreach ($data['datatable'] as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $csz[$key2][$key1] = $val2;
                ...
                $ddz = array();
                foreach ($csz as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $quid = array_keys($val2);
                        if (count($val2) == 1 && $quid[0] == 'id') {
                            if (!in_array($key1, $ddz)) {
                                $ddz[] = $key1;
                            }
                        }
                    }
                ...
                $hqys1 = array();
                foreach ($ddz as $key1 => $val1) {
                    foreach ($csz[$val1] as $key2 => $val2) {
                        $hqys1[$val1] = 'zhibiaomingchen,zhibiaobianma,rowcontens,jine';
                        $quid = array_keys($val2);
                        if (count($val2) > 1) {
                            foreach ($val2 as $key3 => $val3) {
//                        $sjlx[$val1][$key2][]=gettype($val3);
                                if (!preg_match("/([\x81-\xfe][\x40-\xfe])/", $key3)) {
                                    if (empty($hqys1[$val1])) {
                                        $hqys1[$val1] = $key3;
                                    } else {
                                        $hqys1[$val1] = $hqys1[$val1] . ',' . $key3;
                                    }
                                }
                            }
                        }
                ...
                foreach ($hqys1 as $key => $val) {
                    if ($val != 'zhibiaomingchen,zhibiaobianma,rowcontens,jine') {
                        $hqys[$key] = $val;
                    }
                }
                foreach ($csz as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $quid1 = array_keys($val2);
                        if (count($val2) == 1 && $quid1[0] == 'id') {
                            $xgz = M($data['shujukumingcheng'] . $key1)->field($hqys[$key1])->where('masid=' . $pdmsid)->select();
                            foreach ($xgz as $key3 => $val3) {
                                $jz = $xgz[$key3]['zhibiaomingchen'] . '_' . $xgz[$key3]['zhibiaobianma'] . '_' . $xgz[$key3]['rowcontens'];
                                foreach ($val3 as $key4 => $val4) {
                                    if ($key4 != 'zhibiaomingchen' && $key4 != 'zhibiaobianma' && $key4 != 'rowcontens') {
                                        if ((strpos($key4, 'nianfen') || strpos($key4, 'shijian') || strpos($key4, 'time') || strpos($key4, 'riqi') || strpos($key4, 'rq') || $key4 == 'nianfen' || $key4 == 'shijian' || $key4 == 'time' || $key4 == 'riqi' || $key4 == 'rq') && strlen($val4) == 10) {
                                            $csz1[$key1][$val3['rowcontens'] - 1][$key4] = date('Y-m-d H:i:s', $val4);
                                        } else {
                                            $csz1[$key1][$val3['rowcontens'] - 1][$key4] = $val4;
                                        }
                                        $csz2[$key1][$val3['rowcontens'] - 1][$jz] = $xgz[$key3]['jine'];
                                    }
                                }
                            }
                        }
                ...
                foreach ($csz1 as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $csz3[$key1][] = $val2;
                    }
                }
                foreach ($csz3 as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        if (!empty($csz2[$key1][$key2])) {
                            $val2 = $val2 + $csz2[$key1][$key2];
                        }
                        $csz4[$key1][] = $val2;
                    }
                }

                foreach ($csz4 as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        foreach ($val2 as $key3 => $val3) {
                            if ($key3 == 'id') {
                                $qid = $val3;
                                unset($csz4[$key1][$key2][$key3]);
                                $csz4[$key1][$key2]['id'] = $qid;
                            }
                            if ($key3 == 'jine') {
                                unset($csz4[$key1][$key2][$key3]);
                            }
                ...
                foreach ($csz as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $quid = array_keys($val2);
                        if (count($val2) == 1 && $quid[0] == 'id') {
                            $csz[$key1][$key2] = $csz4[$key1][$key2];
                        }
                    }
                }
                foreach ($csz as $key1 => $val1) {
                    foreach ($val1 as $key2 => $val2) {
                        $quidsj = array_keys($data['datatable'][$key2][$key1]);
                        if (count($data['datatable'][$key2][$key1]) == 1 && $quidsj[0] == 'id') {
                            $data['datatable'][$key2][$key1] = $val2;
                        }
                    }
                }
            ...
            //
            foreach ($data['#hide#datatable'] as $keyone => $valueone) //找出是表格的数组
            {
                //$Tepmprowidgrid=
                foreach ($valueone as $key => $val) //得到了一行记录
                {
                    if (in_array($key, $twodimensional))  //是否在二维表字段名
                    {
                        ...
                        {
                            if ($keyend != 'id') {
                                unset($data['#hide#datatable'][$keyone][$key][$keyend]);
                            }
                        }
                    }
                }
            ...
            foreach ($data['#hide#datatable'] as $keyone => $valueone) //找出是表格的数组
            {
                ...
                foreach ($valueone as $key => $val) //得到了一行记录
                {
                    if (in_array($key, $twodimensional))  //是否在二维表字段名
                    {
                        foreach ($val as $keyend => $valend) //得到了具体的字段值
                        {
                            $chazhao = M($data['shujukumingcheng'] . $key)->where('id=' . $valend)->field('masid')->select();
                            $chazhao2[$key] = M($data['shujukumingcheng'] . $key)->where('masid=' . $chazhao[0]['masid'])->field('id,rowcontens')->select();
                            $chazhaoxx[$key] = M($data['shujukumingcheng'] . $key)->where('masid=' . $chazhao[0]['masid'] . ' and rowcontens=' . $chazhao2[$key][0]['rowcontens'])->field('id')->count();
                            $xzks[$key] = count($chazhao2[$key]);
                            $idxh = $idxh + 1;
                        }
                    }
                }
            ...
            foreach ($data['#hide#datatable'] as $keyone => $valueone) //找出是表格的数组
            {
                ...
                foreach ($valueone as $key => $val) //得到了一行记录
                {
                    if (in_array($key, $twodimensional) == false)  //是否在二维表字段名
                    {
                        $chazhao4[$keyone][$key] = $_POST["#hide#datatable"][$keyone][$key];
                    }
                }
            }
            foreach ($chazhao4 as $key1 => $val1) {
                if (empty($val1)) {
                    unset($chazhao4[$key1]);
                }
            }

            foreach ($chazhao2 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chazhao3[$key2][$key1]['id'] = $val2['id'];
                    $yyhs[$key1] = $val2['rowcontens'];
                }
            }
            if (count($chazhao4) > count($chazhao3)) {
                for ($i = 0; $i < count($chazhao4); $i++) {
                    if (empty($chazhao3[$i])) {
                        $data['#hide#datatable'][$i] = $chazhao4[$i];
                    } else {
                        $data['#hide#datatable'][$i] = array_merge($chazhao4[$i], $chazhao3[$i]);
                    }
                }
            } else {
                for ($i = 0; $i < count($chazhao3); $i++) {
                    if (empty($chazhao4[$i])) {
                        $data['#hide#datatable'][$i] = $chazhao3[$i];
                    } else {
                        $data['#hide#datatable'][$i] = array_merge($chazhao3[$i], $chazhao4[$i]);
                    }
                }
            }
            //第一次得到id对应的值
...
//	var_dump($zhibiaomingchen);
            foreach ($data['datatable'] as $keyone => $valueone) //找出是表格的数组
            {
                ...
                foreach ($valueone as $key => $val) //得到了一行记录
                {
                    if (in_array($key, $twodimensional) == false)  //是否在二维表字段名
                    {
                        $xiugaisanci[$keyone][$key] = $_POST["datatable"][$keyone][$key];
                    }
                }
            }
            foreach ($zhibiaomingchen as $key1 => $val1) {
                $beiyong[$key1] = array_values($val1);
            }
            $beiyong1 = array();
            $c = 0;
            foreach ($beiyong as $key1 => $val1) {
                ...
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $key3)) {
                            ...
                            $beiyong1[$key1][$b]['zhibiaomingchen'] = $k2[0];
                            $beiyong1[$key1][$b]['zhibiaobianma'] = $k2[1];
                            $beiyong1[$key1][$b]['jine'] = empty($val3) ? 0 : $val3;
                            $beiyong1[$key1][$b]['rowcontens'] = $k2[2];
                            $ids = M($data['shujukumingcheng'] . $key1)->where('rowcontens=' . $k2[2] . ' and masid=' . $data['id'])->field('id')->select();
                            $beiyong1[$key1][$b]['id'] = $ids[$c]['id'];
                            $c = $c + 1;
                            if ($c >= count($ids)) {
                                $c = 0;
                            }
//                        var_dump(count($ids));
            ...
            //提取新增二维表字段
            foreach ($beiyong1 as $key1 => $val1) {
                if (in_array($key1, $twodimensional) && empty($xzks[$key1])) {
                    $byxz[$key1] = $val1;
                    unset($beiyong1[$key1]);
                    $byxz1[$key1] = $beiyong[$key1];
                    unset($beiyong[$key1]);
                }
            }

            foreach ($byxz as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    if (empty($val2['id'])) {
                        $byxz[$key1][$key2]['id'] = 0;
                    }
            ...
            foreach ($byxz1 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $byxz1[$key1][$key2]['rowcontens'] = $key2 + 1;
                    if (empty($byxz1[$key1][$key2]['id'])) {
                        unset($byxz1[$key1][$key2]['id']);
                        $byxz1[$key1][$key2]['id'] = 0;
                    }
                }
            }
            $zd = array();
            ...
            foreach ($byxz as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    if (in_array($val2['zhibiaomingchen'], $zd[$key1])) {
                        continue;
                    }
                    $zd[$key1][$key2] = $val2['zhibiaomingchen'];
                }
            }
            foreach ($zd as $key1 => $val1) {
                $xzyhsl[$key1] = count($zd[$key1]);
            }
            foreach ($byxz1 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    for ($i = 0; $i < $xzyhsl[$key1]; $i++) {
                        $byxz2[$key1][$xzyhsl[$key1] * $key2 + $i] = $val2;
                        if ($byxz2[$key1][$xzyhsl[$key1] * $key2 + $i]['id'] != 0) {
                            $byxz2[$key1][$xzyhsl[$key1] * $key2 + $i]['id'] = $val2['id'] + $i;
                        }
                    }
                }
            }
            foreach ($byxz2 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if ($key3 != 'id') {
                            $byxz[$key1][$key2][$key3] = $val3;
                        }
                    }
                }
            }
            $byxz3 = array();
            foreach ($byxz as $key => $val) {
                foreach ($val as $key1 => $val1) {
                    $byxz3[$key1][$key] = $val1;
                }
            }
            foreach ($byxz3[0] as $key1 => $val1) {
//            var_dump($key1);
                $listerweiid = M($_POST['shujukumingcheng'] . $key1)->where('masid=' . $_POST['masid'])->field('id')->select();
                foreach ($listerweiid as $key2 => $val2) {
                    $table = $_POST['shujukumingcheng'] . $key1;
                    $idew = $val2['id'];
                    $modelname = $_POST['shanchsj'][$key1];
//                $name=$this->getActionName();
                    $map['id'] = array('eq', $idew);
                    $model = M($table);
                    $this->_over_delete($modelname, $id);
                    $model->where($map)->delete();
                }
            }
            foreach ($byxz3 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $byxz3[$key1][$key2]['id'] = 0;
                }
            }
//        var_dump($byxz3);
//        exit;
            foreach ($beiyong as $key1 => $val) {
                foreach ($val as $key2 => $val2) {
//                for($i=0;$i<count($beiyong1[$key1]);$i++){
//                    $beiyong2[$key1][$i]=$val[$c];
//                    $d=$d+1;
//                }
                    $beiyong2[$key1][($key2 + 1) * $chazhaoxx[$key1]] = $val2;
                }
            ...
            foreach ($beiyong3 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    foreach ($val2 as $key3 => $val3) {
                        if ($key3 != 'id' && $key3 != 'jine') {
                            $beiyong1[$key1][$key2][$key3] = $val3;
                        } elseif (empty($beiyong1[$key1][$key2][$key3]) && $key3 == 'id') {
                            $beiyong1[$key1][$key2][$key3] = 0;
                        }
                    }
                }
            }
            ...
            foreach ($beiyong1 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val3) {
                    if (empty($beiyong1[$key1][$key2]['rowcontens'])) {
                        if ($bt != $key1) {
                            $b = 1;
                            $c = 0;
                        }
                        $bt = $key1;
                        $beiyong1[$key1][$key2]['rowcontens'] = $yyhs[$key1] + $b;
                        $c = $c + 1;
                        if ($c >= $chazhaoxx[$key1]) {
                            $b = $b + 1;
                            $c = 0;
...
//        var_dump($data['datatable']);
            //第一次得到$data['datatable']的值
            foreach ($data['#hide#datatable'] as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chongxie02_nu[$key2][$key1] = $val2['id'];
                }
            }
            foreach ($data['datatable'] as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chongxie01_nu[$key2][$key1] = $val2['id'];
                }
            }
            $chongxie01 = $data['datatable'];
            $chongxie02 = $data['#hide#datatable'];
            foreach ($data['datatable'] as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    if (empty($chongxie02_nu[$key2])) {
                        $chongxie03[$key1][$key2] = $data['datatable'][$key1][$key2];
                    }
                }
            }
            foreach ($chongxie02_nu as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    for ($i = 0; $i < count($val1); $i++) {
                        if ($val2 == $chongxie01_nu[$key1][$i]) {
                            $chongxie03[$key2][$key1] = $chongxie01[$i][$key1];
                            unset($chongxie02[$key2][$key1]);
                            if (empty($chongxie02[$key2])) {
                                unset($chongxie02[$key2]);
                            }
                        } elseif ($val2 !== $chongxie01_nu[$key1][$i] && isset($chongxie01[$i][$key1])) {
                            $chongxie04[$i][$key1] = $chongxie01[$i][$key1];
                            if (empty($chongxie04[$i][$key1]['id'])) {
                                $chongxie04[$i][$key1]['id'] = 0;
                            } else {
                                unset($chongxie04[$i][$key1]);
                            }
                        }
                    }
                }
            ...
            foreach ($chongxie04 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chongxie05[$xzks[$key2] + $key1][$key2] = $val2;
                }
            }
            ksort($chongxie05);
            foreach ($chongxie05 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chongxie06[$key2][$key1] = $val2;
                }
            }
//        var_dump($chongxie04);
//        exit;
            foreach ($chongxie06 as $key1 => $val1) {
                foreach ($val1 as $key2 => $val2) {
                    $chongxie07[$key2][$key1] = $val2;
                }
            }
//        var_dump($chazhaoxx);
//        exit;
            unset($data['#hide#datatable']);
            $data['#hide#datatable'] = $chongxie02;
            if (empty($data['#hide#datatable'])) {
                unset($data['#hide#datatable']);
            }
            unset($data['datatable']);
            if (empty($chongxie03) && isset($chongxie07)) {
                $chongxie08 = $chongxie07;
            } elseif (empty($chongxie07) && isset($chongxie03)) {
                $chongxie08 = $chongxie03;
            } elseif (isset($chongxie03) && isset($chongxie07)) {
                $chongxie08 = $chongxie03 + $chongxie07;
            } else {
                $chongxie08 = '';
            }
            if (empty($chongxie08)) {
            ...
            if (count($xiugaisanci) > count($chongxie08)) {

                foreach ($chongxie08 as $key1 => $val1) {
                    if (isset($xiugaisanci)) {
                        $chongxie08[$key1] = array_merge($xiugaisanci[$key1], $chongxie08[$key1]);
                    } else {
                        $xiugaisanci[$key1] = $chongxie08[$key1];
                    }
                }
                ...
                foreach ($xiugaisanci as $key1 => $val1) {
                    if (isset($chongxie08[$key1])) {
                        $chongxie08[$key1] = array_merge($chongxie08[$key1], $xiugaisanci[$key1]);
                    } else {
                        $chongxie08[$key1] = $xiugaisanci[$key1];
                    }
            ...
            if (empty($byxz3) && isset($xgsj)) {
                $data['datatable'] = $xgsj;
            } elseif (empty($xgsj) && isset($byxz3)) {
                $data['datatable'] = $byxz3;
            } elseif (isset($xgsj) && isset($byxz3)) {
                if (count($byxz3) > count($xgsj)) {

                    foreach ($xgsj as $key1 => $val1) {
                        if (isset($byxz3[$key1])) {
                            $byxz3[$key1] = array_merge($byxz3[$key1], $xgsj[$key1]);
                        } else {
                            $byxz3[$key1] = $xgsj[$key1];
                        }
                    }
                    ...
                    foreach ($byxz3 as $key1 => $val1) {
                        if (isset($xgsj[$key1])) {
                            $xgsj[$key1] = array_merge($xgsj[$key1], $byxz3[$key1]);
                        } else {
                            $xgsj[$key1] = $byxz3[$key1];
                        }
...
//        var_dump($data['datatable']);
        if (in_array(1, $data['istwodimensional'])) {
            unset($_POST);
            $_POST = $data;
        }

//       var_dump($data);
//        var_dump("_____________________________________");
//        var_dump($data['#hide#datatable']);
//        var_dump($_POST);
//         exit
//        exit;
        //获取当前数据ID
...
//        $this->display ("Public:exportBysearchHtml");
        ...
        if ($modelname == 'MisSalesMyProject') {
            $modelname = "MisAutoPvb";
        }
        $filearr = require DConfig_PATH . "/Models/" . $modelname . "/list.inc.php";
        ...
        if (!empty($PrivateListincList)) {
            ...
            foreach ($filearr as $fk => $fv) {
                ...
                if ($isexisit) {
                    ...
                    $filearr[$fk]['yangshi'] = $PrivateListincList[$fk]['yangshi'];
                }
            }
        } else {
            //读取公共模块设置
            ...
            if (!empty($PublicListincList)) {
                ...
                foreach ($filearr as $fk => $fv) {
                    ...
                    if ($isexisit) {
                        ...
                        $filearr[$fk]['yangshi'] = $PublicListincList[$fk]['yangshi'];
                    }
                }
            }
        }
...
//        exit;
        ...
        $filearr = require DConfig_PATH . "/Models/" . $name . "/list.inc.php";
        //读取个人是否设置如果有则读取个人设置 如果不存在个人设置则读取公共设置
        $PrivateListincModel = M('mis_system_private_listinc');
        $PrivateListincMap['modelname'] = $name;
        $PrivateListincMap['userid'] = $_SESSION[C('USER_AUTH_KEY')];
        $PrivateListincList = $PrivateListincModel->where($PrivateListincMap)->select();
        foreach ($PrivateListincList as $pk => $pv) {
            $PrivateListincList[$pv['fieldname']] = $pv;
        ...
        if (!empty($PrivateListincList)) {
            ...
            //读取公共模块设置
            $PublicListincModel = M('mis_system_public_listinc');
            $PublicListincMap['modelname'] = $name;
            $PublicListincList = $PublicListincModel->where($PublicListincMap)->select();
            foreach ($PublicListincList as $pk => $pv) {
                $PublicListincList[$pv['fieldname']] = $pv;
            }
        ...
        foreach ($filearr as $k => $v) {
            if ($v['shows'] == 1 && in_array($v['name'], $field)) {
                $data_head[$v['name']] = $v['showname'];
            }
        }
        $data = array();
...
//            dump($dataArr);exit;
            ...
            $tablename = getFieldBy($actionname, 'modelname', 'tablename', 'mis_dynamic_database_mas');
            ...
            foreach ($dataArr as $key => $val) {
                ...
                // 实例地区信息表
                $model = M('mis_dynamic_form_areainfo');
                $data = '';
...
// 			header("Content-Disposition: attachment; filename*=utf8''".basename(str_replace("+", "%20", urlencode($info['basename']))));
// 			ob_clean();
// 			flush();
// 			readfile($path);
            ...
            readfile(iconv('UTF-8', 'GBK//IGNORE', $path));
    ...
    public function lookupGenerals()
    {
        ...
        $lookupGruop = $_REQUEST['lookupgroup'];
        //lookup再选带回时验证表格是否有值参数
        $lookupcheck = $_REQUEST['lookupcheck'];
...
AND STATUS=1";
        ...
        if ($lookupKey == '2ba96fc5ab7bef0ac9a4d3f5d72bb1f0') {
            ...
            $deptlist = $model->where($departmentmap)->order("sort asc")->select();
        ...
        if ($viewname) {
            ...
            $action->_viewlist($viewname, $map, $sortBy = '', $asc = false, $group = '', '', $sortstr, 1);
        } else {
        ...
        if ($lookupKey == '2ba96fc5ab7bef0ac9a4d3f5d72bb1f0' || $lookupKey == 'a945d1a1f0487f8621c1d7797b894295') {//2ba96fc5ab7bef0ac9a4d3f5d72bb1f0 b6fe179d38c8034cee79b5b09dd6b1e7
            ...
            if ($lookupDetail ['dt']) { // 内嵌表 页面 带回多条记录
                ...
                if ($lookupcheck == "contextc") {
                    //跳转到提醒界面
                    $this->display('Public:lookupcheck');
                } else {
                ...
                if ($lookupDetail['tree']) {
                    ...
                    //判断是业务受理模块，跳转到有新增客户的的lookupGeneralsAdd页面
                    if ($lookupGruop == 'ORG27810') {
                        $this->display('Public:lookupGeneralsAdd');
                    } else {
                        $this->display('Public:lookupGenerals');
                    }
                }
...
// 			$this->assign("viewname",$_REQUEST['viewname']);
    ...
    {
        //增加操作符+操作值功能
        $htjq['id'] = $_REQUEST['zbz'];
        $qz = M('mis_system_lookupobj')->field('inlineinfo')->where($htjq)->select();
        $qz1 = json_decode($qz[0]['inlineinfo'], true);
        $qid = '';
        $cxyj = array();
        foreach ($qz1['selectfuncdata'] as $key1 => $val1) {
            foreach ($val1 as $key2 => $val2) {
                if (!empty($val2) && !empty($qz1['czffuncdata'][$key1][$key2])) {
                    if (empty($qid)) {
                        $qid = $key2;
                    } else {
                        $qid = $qid . ',' . $key2;
                    ...
                    if ($val2 == 1) {
                        $val2 = 'like';
                        $qz1['czffuncdata'][$key1][$key2] = $qz1['czffuncdata'][$key1][$key2] . '%';
                    } elseif ($val2 == 2) {
                        $val2 = 'like';
                        $qz1['czffuncdata'][$key1][$key2] = '%' . $qz1['czffuncdata'][$key1][$key2];
                    } elseif ($val2 == 'like') {
                    ...
                    if ($qz1['czffuncdata'][$key1][$key2] == 'empty') {
                        $qz1['czffuncdata'][$key1][$key2] = '';
                    }

                    $hqz[] = $val2;
                    $hqz1[] = $qz1['czffuncdata'][$key1][$key2];
                }
            }
        }

        if (!empty($qid)) {
            if (!empty($qid) && strpos($qid, ',')) {
                $qdzz = M('mis_dynamic_form_datatable')->field("fieldname")->where(array('id' => array('in', $qid)))->select();
            } elseif (!empty($qid) && !strpos($qid, ',')) {
                $qdzz = M('mis_dynamic_form_datatable')->field("fieldname")->where(array('id' => $qid))->select();
            }
            foreach ($hqz as $key1 => $val1) {
                $cxyj[$qdzz[$key1]['fieldname']] = array($val1, $hqz1[$key1]);
            }
        }
        //增加结束
...
//         			  		echo $funcKey.$fieldVal."--";
            ...
            if (strpos($jsonArr['xifenpinlei'], ',')) {
                $pieces = explode(",", $jsonArr['xifenpinlei']);
                $duogezhi = M('mis_sale_profession')->field('name,orderno')->select();
                foreach ($duogezhi as $key1 => $val1) {
                    $duogezhi1[$val1['orderno']] = $val1['name'];
                }
                foreach ($pieces as $key1 => $val1) {
                    $pieces1[] = $duogezhi1[$val1];
                }
                $jsonArr['xifenpinlei'] = implode(",", $pieces1);
                $pieces1 = array();
            }
...
// 			$ip = gethostbyname($_SERVER['SERVER_NAME']);
            ...
            require_once("http://{$ip}:37788/JavaBridge/java/Java.inc");//此行必须
            $PageOfficeCtrl = new Java("com.zhuozhengsoft.pageoffice.PageOfficeCtrlPHP");//此行必须
            $PageOfficeCtrl->setServerPage("http://{$ip}:37788/JavaBridge/poserver.zz");//此行必须，设置服务器页面
...
// 			$PageOfficeCtrl->webOpen($file_path, $OpenMode->docNormalEdit, "张三");//此行必须
        ...
        } elseif ($file_extension_lower == "xls" || $file_extension_lower == "xlsx") {
            $action = $this->getActionName();
            $ip = C("DB_HOST_WORD");//"192.168.0.238";
            require_once("http://{$ip}:37788/JavaBridge/java/Java.inc");//此行必须
            $PageOfficeCtrl = new Java("com.zhuozhengsoft.pageoffice.PageOfficeCtrlPHP");//此行必须
            $PageOfficeCtrl->setServerPage("http://{$ip}:37788/JavaBridge/poserver.zz");//此行必须，设置服务器页面
            $url = __ROOT__ . "/Admin/index.php/$action/onlineReadExcel/name/" . $_REQUEST['name'];
            java_set_file_encoding("UTF-8");//设置中文编码，若涉及到中文必须设置中文编码
            //打开excel文档
            $PageOfficeCtrl->UserAgent = $_SERVER['HTTP_USER_AGENT'];//若使用谷歌浏览器此行代码必须有，其他浏览器此行代码可不加
            $OpenMode = new Java("com.zhuozhengsoft.pageoffice.OpenModeType");
            ...
            //获取用户名称
            $loginUserName = getFieldBy($uid, "id", "name", "user");
            $PageOfficeCtrl->webOpen($file_path, $OpenMode->xlsReadOnly, $loginUserName);//此行必须
            $this->assign('PageOfficeCtrl', $PageOfficeCtrl->getDocumentView("PageOfficeCtrl1"));
            $this->display("Public:showReadExcel");
        } else {
            ...
            if (!empty($PublicListincListInfo)) {
                ...
                foreach ($list as $k => $v) {
                    ...
                    if ($isexisit) {
                        ...
                        $list[$k]['yangshi'] = $PublicListincListInfo[$v['name']]['yangshi'];
                    }
                }
            }
                ...
                if (!empty($privateListincListInfo)) {
                    foreach ($list as $k => $v) {
                        ...
                        if ($isexisit) {
                            ...
                            $list[$k]['yangshi'] = $privateListincListInfo[$v['name']]['yangshi'];
                        }
                    }
                } else {
                    //读取公共模块设置
        ...
        if ($_SESSION['a']) {
            ...
            foreach ($showname as $shownamek => $shownamev) {
                ...
                if ($publistListincList[$shownamek] == null) {
                    ...
                    $publistData['yangshi'] = $yangshi[$shownamek];
            ...
            foreach ($showname as $shownamek => $shownamev) {
                ...
                if (empty($privateListincList[$shownamek])) {
                    ...
                    $privateData['yangshi'] = $yangshi[$shownamek];
        ...
        if ($is_exists) {
            ...
            if (is_writable($docfile)) {
                ...
                if (!$handle = fopen($htmlFileSaveName, 'w+')) {
                    echo 0;
                    exit;//不能打开文件 $htmlFileSaveName
                }
                if (fwrite($handle, $html) === FALSE) {
                    echo 0;
                    exit;//不能写入到文件 $htmlFileSaveName
                }
                fclose($handle);
            }
        }
...
// 					echo $model->getDbError();
	...
	*/
        ...
        foreach ($detailList as $k2 => $v2) {
            ...
            if ($vo) {
                if (isset($v2['fieldcategory']) && $v2['fieldcategory'] == 'textarea') {
                    ...
                    if (isset($v2['fieldcategory']) && $v2['fieldcategory'] == 'iframe') {
                        $valArr = array('name' => $fielename, 'showname' => $v2['showname'], 'istestarea' => 1, 'original' => $vo[$v2['name']], 'value' => $vo[$v2['name']]);
                    }
    ...
    public function getDetaileZhList($detailList, $volist, $fieldname, $showname)
    {
        ...
        foreach ($detailList as $k2 => $v2) {
            if ($v2['fieldcategory'] == "uploadfilenew" || $v2['name'] == "id" || $v2['name'] == "action" || $v2['name'] == "auditState" || strpos(strtolower($v2['name']), "datatable") !== false) {
                continue;
            }
            $val = array();
            $original = array();
...
// 							print_r($orderSql);exit;
     ...
     * @throws
     */
    function delsubinfo($table, $idew, $modelname, $name)
    {
//        var_dump($_POST);
//        exit;
        if (!empty($table) && !empty($idew) && !empty($modelname) && !empty($name)) {
            $table = $table;
            $id = $idew;
            $modelname = $modelname;
            $name = $name;
            ...
            $name = $this->getActionName();
            if (isset($_GET['erwei']) && $_GET['erwei'] == 1) {
                $chazhao = M($table)->where('id=' . $id)->field('masid,rowcontens')->select();
                $chazhao2 = M($table)->where('masid=' . $chazhao[0]['masid'] . ' and rowcontens=' . $chazhao[0]['rowcontens'])->field('id')->select();
//            var_dump($chazhao2);
//            exit;
                foreach ($chazhao2 as $key1 => $val1) {
                    $id = $val1['id'];
            ...
            if ($rval['id']) {
                if ($rval['typeid'] == 2) {
                    ...
                    if ($rval['id'] == -1) {
                        ...
                        //edit页面
                        if ($rval['actionname'] == 'MisAutoByp') {
                            $tablesHtml .= "<li {$selected}><a id=\"{$rval['actionname']}_#tagitem#\" class=\"j-ajax\" rel=\"{$main}{$rval['actionname']}_edit\" href=\"" . __APP__ . "/{$rval['actionname']}/view/fieldtype/{$rval['fieldtype']}/{$rval['fieldtype']}/{$val}/id/{$rval['id']}/bindaname/{$action}{$mainBindActionCondition}\">";
                        } else {
...
// 	}
    ...
    public function DT_uploadnew()
    {
        ...
        foreach ($list as $key => $val) {
            $uptype = substr(strrchr($val['upname'], '.'), 1);
            if (in_array($uptype, $jpgArr)) {
                $list[$key]['uptype'] = 1;
            } else {
                $list[$key]['uptype'] = 2;
            }
        ...
        $this->assign("list", $list);
        //判断是否出现上传按钮
        $shangchuan = empty($_REQUEST['shangchuan']) ? 0 : 1;
        $this->assign("shangchuan", $shangchuan);
...
// // 			echo M()->getLastSql();
     ...
     * @throws
     */
    protected function unsetOldDataToCache($updateBackup)
    {
...
// 		M()->execute("DELETE FROM  ".$randtable." WHERE randnum='".$randnum."' and createid='".$_SESSION[C('USER_AUTH_KEY')]."';");
    ...
    {
        $IframeBandaname['bindaname'] = $this->getActionName();
        $IframeInBandanmes = M('mis_auto_bind')->where($IframeBandaname)->field('inbindaname')->select();
        $IframeInBandanme = $IframeBandaname['bindaname'];
        foreach ($IframeInBandanmes as $key1 => $val1) {
            $IframeInBandanme = $IframeInBandanme . ',' . $val1['inbindaname'];
        }
        if ($IframeInBandanme == $IframeBandaname['bindaname']) {
            $WordName['modelname'] = $IframeInBandanme;
        } else {
            $WordName['modelname'] = array('in', $IframeInBandanme);
        }
        $WordNqb = M('mis_dynamic_database_mas')->where($WordName)->field('modelname,formid')->select();
        $WordTables = array();
        foreach ($WordNqb as $key1 => $val1) {
            $WordTblk['formid'] = $val1['formid'];
            $WordTblk['category'] = 'lookup';
            $WordTables1 = M('mis_dynamic_form_datatable')->where($WordTblk)->field('fieldname,fieldtitle,config,tablename')->select();
            $geshu = count($WordTables);
            for ($i = 0; $i < count($WordTables1); $i++) {
                $WordTables[$geshu + $i] = $WordTables1[$i];
            }
        }
        $WordLookUp2 = array();
        foreach ($WordTables as $key1 => $val1) {
            foreach ($val1 as $key2 => $val2) {
                if ($key2 == 'tablename') {
                    $neiqiantabelname = explode("_", $val2);
                    $WordNqPass = "";
                    foreach ($neiqiantabelname as $va) {
                        $WordNqPass .= ucwords($va);
                    }
                    $WordNqPass1['tablename'] = $WordNqPass;
                    $misformdatasort = M("mis_dynamic_form_datatable_sort")->where($WordNqPass1)->field("modelname,sort")->find();
                    $mdstrt = substr(md5("datatablebdnq" . $misformdatasort['modelname'] . $misformdatasort['sort']), 0, 8);
                    $mdstrtt = substr(md5("datatabletbnq" . $WordNqPass . $misformdatasort['sort']), 0, 8);
                    $mdstrttone = substr(md5("datatablebdnq" . $WordNqPass . $misformdatasort['sort']), 0, 8);
                    $mdstrttname = substr(md5($val1['fieldname']), 0, 8);
                    $mdstrt1 = substr(md5($mdstrt . $mdstrttname), 0, 8);
                    $mdstrtt1 = substr(md5($mdstrtt . $mdstrttname), 0, 8);
                    $mdstrttone1 = substr(md5($mdstrtt . $mdstrttone), 0, 8);
                    $fanxul = unserialize(base64_decode($val1['config']));
//                    mis_system_lookupobj
                    $NqLookUp['id'] = $fanxul['parame'][0];
                    $LookUpNq = M('mis_system_lookupobj')->where($NqLookUp)->field('mode')->find();

                    if (!empty($WordLookUp2[$mdstrtt])) {
                        $Nummdstrtt = count($WordLookUp2[$mdstrtt]);
                        $WordLookUp2[$mdstrtt][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrtt1][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrttone][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrttone1][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrt][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        //$mdstrt1
                        $WordLookUp2[$mdstrt1][$Nummdstrtt] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                    } else {
                        $WordLookUp2[$mdstrtt][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrtt1][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrttone][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrttone1][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrt][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                        $WordLookUp2[$mdstrt1][0] = array(
                            'fieldtitle' => $val1['fieldtitle'],
                            'modelname' => $LookUpNq['mode']
                        );
                    }
                    //var_dump("datatablebdnq".$WordNqPass.$misformdatasort['sort']);
                }
            }
        }
        set_time_limit(0);
...
// 						$ip = gethostbyname($_SERVER['SERVER_NAME']);
        ...
        $WordNqb = M('mis_dynamic_database_mas')->where($WordName)->field('tablename')->select();
        $WordTables = array();
        foreach ($WordNqb as $key1 => $val1) {
            $WordTblk['tablename'] = array('like', $val1['tablename'] . '%', 'and');
            $WordTblk['category'] = 'lookup';
            $WordTables1 = M('mis_dynamic_form_datatable')->field("id,fieldname,fieldtitle,category,config,tablename")->where($WordTblk)->select();
            $geshu = count($WordTables);
            for ($i = 0; $i < count($WordTables1); $i++) {
                $WordTables[$geshu + $i] = $WordTables1[$i];
            }
        ...
        //组装内嵌表加密后数组
        $Bigeiqiantabelname = array();
        foreach ($WordTables as $key => $val) {
            $neiqiantabelname = explode("_", $val['tablename']);
            $strtt = "";
            foreach ($neiqiantabelname as $va) {
                $strtt .= ucwords($va);
            }
            array_push($Bigeiqiantabelname, $strtt);
        }
        $misform_datatable_sort = M("mis_dynamic_form_datatable_sort");
        $misformdatasort = $misform_datatable_sort->field("tablename,sort")->select();
        $mdmistr = array();//内嵌表加密后数组
        foreach ($misformdatasort as $key => $val) {
            foreach ($Bigeiqiantabelname as $va) {
                if ($val['tablename'] == $va) {
                    $mdstrtt = "";
                    $mdstrtt = "datatabletbnq" . $val['tablename'] . $val['sort'];
                    $mdstrtt = substr(md5($mdstrtt), 0, 8);
                    array_push($mdmistr, $mdstrtt);
                }
            }
        }
//        foreach($mdmistr as $va){
//            foreach($list as $key=>$val){
//                if($va == $key){
//                    var_dump($list[$key]);
//                }
...
//        }
        ...
        foreach ($list as $key0 => $val0) {
            if (!empty($WordLookUp2[$key0])) {
                if (count($WordLookUp2[$key0]) > 1) {
                    if (empty($val0['value'][0]['showname'])) {
                        $wvalue1 = array();
                        for ($wi = 0; $wi < count($WordLookUp2[$key0]); $wi++) {
                            if ($val0['showname'] == $WordLookUp2[$key0][$wi]['fieldtitle'] || strpos($val0['showname'], $WordLookUp2[$key0][$wi]['fieldtitle'])) {
                                if (!empty($val0['original'])) {
                                    if (count($val0['original']) == 1) {
                                        $wlookup['orderno'] = implode(',', $val0['original']);
                                        $wvalue11 = D($WordLookUp2[$key0][$wi]['modelname'])->where($wlookup)->field('name')->find();
                                        $wvalue1[0] = $wvalue11['name'];
                                    } elseif (count($val0['original']) > 1) {
                                        foreach ($val0['original'] as $wkey2 => $wval2) {
                                            $wlookup['orderno'] = $wval2;
                                            $wvalue11 = D($WordLookUp2[$key0][$wi]['modelname'])->where($wlookup)->field('name')->find();
                                            $wvalue1[] = $wvalue11['name'];
                                        }
                                    }
                                }
                                $list[$key0]['value'] = $wvalue1;
                            }
                        }
                    } else {
                        foreach ($val0['value'] as $wkey1 => $wval1) {
                            $wvalue1 = array();
                            for ($wi = 0; $wi < count($WordLookUp2[$key0]); $wi++) {
                                if ($wval1['showname'] == $WordLookUp2[$key0][$wi]['fieldtitle'] || strpos($wval1['showname'], $WordLookUp2[$key0][$wi]['fieldtitle'])) {
                                    if (!empty($wval1['original'])) {
                                        if (count($wval1['original']) == 1) {
                                            $wlookup['orderno'] = implode(',', $wval1['original']);
                                            $wvalue11 = D($WordLookUp2[$key0][$wi]['modelname'])->where($wlookup)->field('name')->find();
                                            $wvalue1[0] = $wvalue11['name'];
                                        } elseif (count($wval1['original']) > 1) {
                                            foreach ($wval1['original'] as $wkey2 => $wval2) {
                                                $wlookup['orderno'] = $wval2;
                                                echo 3;
                                                $wvalue11 = D($WordLookUp2[$key0][$wi]['modelname'])->where($wlookup)->field('name')->find();
                                                $wvalue1[] = $wvalue11['name'];
                                    ...
                                    $list[$key0]['value'][$wkey1]['value'] = $wvalue1;
                                }
                            }
                        }
                    }

                } elseif (count($WordLookUp2[$key0]) == 1) {
                    $wvalue1 = array();
                    if (empty($val0['value'][0]['showname'])) {
                        if ($val0['showname'] == $WordLookUp2[$key0][0]['fieldtitle'] || strpos($val0['showname'], $WordLookUp2[$key0][0]['fieldtitle'])) {
                            if (!empty($val0['original'])) {
                                if (count($val0['original']) == 1) {
                                    $wlookup['orderno'] = implode(',', $val0['original']);
                                    $wvalue11 = D($WordLookUp2[$key0][0]['modelname'])->where($wlookup)->field('name')->find();
                                    $wvalue1[0] = $wvalue11['name'];
                                } elseif (count($val0['original']) > 1) {
                                    foreach ($val0['original'] as $wkey2 => $wval2) {
                                        $wlookup['orderno'] = $wval2;
                        ...
                        foreach ($val0['value'] as $wkey1 => $wval1) {
                            $wvalue1 = array();
                            if ($wval1['showname'] == $WordLookUp2[$key0][0]['fieldtitle'] || strpos($wval1['showname'], $WordLookUp2[$key0][0]['fieldtitle'])) {
                                if (!empty($wval1['original'])) {
                                    if (count($wval1['original']) == 1) {
                                        $wlookup['orderno'] = implode(',', $wval1['original']);
                                        $wvalue11 = D($WordLookUp2[$key0][0]['modelname'])->where($wlookup)->field('name')->find();
            ...
            if (!empty($val0['value'])) {
                foreach ($val0['value'] as $key00 => $val00) {
                    if ($val00['showname'] == 'rowcontens') {
                        $ceshiwordhs['row'] = count($val00['value']) / count(array_unique($val00['value']));
                        unset($list[$key0]['value'][$key00]);
                        foreach ($val0['value'] as $key1 => $val1) {
                            if ($val1['showname'] == '指标名称') {
                                foreach (array_unique($val1['value']) as $key2 => $val2) {
                                    $ceshiwordzbmc[$key2] = array(
                                        'name' => $val1['name'] . $key2,
                                        'is_stats' => $val1['is_stats'],
                                        'func' => $val1['func'],
                                        'funcdata' => $val1['funcdata'],
                                        'showname' => $val2
                                    );
                                }
                                unset($list[$key0]['value'][$key1]);
                            }
                            if ($val1['showname'] == '金额') {
                                foreach ($val1['value'] as $key3 => $val3) {
                                    $ceshiwordzbmc1[$key3 % $ceshiwordhs['row']]['original'][floor($key3 / $ceshiwordhs['row'])] = $val3;
                                    $ceshiwordzbmc1[$key3 % $ceshiwordhs['row']]['value'][floor($key3 / $ceshiwordhs['row'])] = $val3;
                                }
                                unset($list[$key0]['value'][$key1]);
                            }
                            if ($val1['showname'] == '指标编码') {
                                unset($list[$key0]['value'][$key1]);
                            }
                        }
                        foreach ($list[$key0]['value'] as $key1 => $val1) {
                            if ($val1['showname'] != '指标名称' && $val1['showname'] != '指标编码' && $val1['showname'] != '指标编码' && $val1['showname'] != '指标编码') {
                                foreach ($val1['value'] as $key2 => $val2) {
                                    $list[$key0]['value'][$key1]['original'][floor($key2 / $ceshiwordhs['row'])] = $val2;
                                    $list[$key0]['value'][$key1]['value'][floor($key2 / $ceshiwordhs['row'])] = $val2;
                                    $zghs = floor($key2 / $ceshiwordhs['row']);
                                }
                                foreach ($val1['value'] as $key3 => $val3) {
                                    if ($key3 > $zghs) {
                                        unset($list[$key0]['value'][$key1]['value'][$key3]);
                                        unset($list[$key0]['value'][$key1]['original'][$key3]);
                                    }
                                }
                            }
                        }
                        foreach ($ceshiwordzbmc as $keycs => $valcs) {
                            $ceshiwordzbmc[$keycs] = array_merge($valcs, $ceshiwordzbmc1[$keycs]);
                        }
                        $list[$key0]['value'] = array_merge($list[$key0]['value'], $ceshiwordzbmc);
                    }
                }
            }
        }
...
// 			$ip = GetHostByName($_SERVER['SERVER_NAME']);//获取本机IP
            ...
            //zip文件名
            $bklne = M("mis_auto_bklne");
            $model = D($modelname);
            $projectid = $model->field('projectid')->where("id=" . $id)->find();
            $projectOrder = $bklne->field("orderno")->where("projectid=" . $projectid["projectid"])->find();
            //获取要导出到压缩包文件里面的modelname
            $flowForm = M("mis_project_flow_form");
            $flowFormWhere['projectid'] = $projectid["projectid"];
            $newdata = $flowForm->field("orderno,name,formobj")->where($flowFormWhere)->select();
            //清除formobj为空的数据
            ...
            $filenameZip = UPLOAD_SampleWord . "_" . $projectOrder['orderno'] . ".zip";
            $zip = new ZipArchive();
            //创建压缩文件
            unlink($filenameZip);
            $filenameZip = UPLOAD_SampleWord . "_" . $projectOrder['orderno'] . ".zip";
//            $filenameZip = iconv("UTF-8","GBK",$filenameZip);
            if (!$zip->open($filenameZip, ZIPARCHIVE::CREATE)) {
                exit("cannot open <$filenameZip>\n");
            } else {
                $zipUrls = array();//地址数组
                foreach ($newdata as $key => $val) {
                    //生成文件的目标目录
                    $causepath = UPLOAD_SampleWord . $val['formobj'];
                    //验证最新文件目录是否存在
                    if (!file_exists($causepath)) {
                        //不存在则生成最新文件目录
                        $this->createFolders($causepath); //判断目标文件夹是否存在
                    ...
                    //模板标签文件地址
                    $template_word = UPLOAD_SampleWord . $val['formobj'] . ".docx";
                    //模板标签
                    $templateModel = D("MisSystemTempleteManage");
                    //查询条件
                    $where["modelname"] = $val['formobj'];
                    $temp = $templateModel->where($where)->select();
                    $findList = array();
                    $modelword = D($val['formobj']);
                    if ($temp) {
                        foreach ($temp as $k => $v) {
                            if ($v["rules"]) {
                                $v["rules"] = str_replace("&#39;", "'", html_entity_decode($v["rules"]));
                                ...
                                $map = "";
                                $map['_string'] = $v["rules"];
                                $findList = $modelword->where($map)->find();
                                if ($findList) {
                                    $findList = $v;
                                    break;
                                }
                        ...
                        if (empty($findList)) {
                            $map2["zhuangtai"] = 1;
                            $map2["modelname"] = $val['formobj'];
                            $findList = $templateModel->where($map2)->find();
                        }
                    }
                    if (!empty($findList)) {
                        $attachedRecord = $this->getAttachedRecordList($findList["id"], true, true, "MisSystemTempleteManage", 0, false);
                    ...
                    if (!file_exists($template_word)) { //模板文件不存在
                        ...
                        $PHPWord = new PHPWord();
                        $section = $PHPWord->createSection();
                        $section->addText($val['name'] . "温馨提示:模板文件不存在,请在模板管理里先上传一个模板", array('color' => 'FF0000', 'size' => 18, 'bold' => true));
                        $section->addTextBreak();
                        $section->addTextBreak();
                        $objWriter = PHPWord_IOFactory::createWriter($PHPWord, "Word2007");
                        $objWriter->save($template_word);
                    ...
                    //生成文件名及地址
                    $savefilename = $val['name'] . $val['orderno'] . "_" . $projectid["projectid"] . '.' . $filepath['extension'];
                    $url = UPLOAD_SampleWord . $val['formobj'] . "/" . "_" . $savefilename;
                    $filenameUTF8Quan = iconv("UTF-8", "GBK", $url);
                    unlink($filenameUTF8Quan);
                    //生成文件
                    $document = $PHPWord->loadTemplate($template_word);
                    $document->clearAllBiaoji();
                    $idmodel = D($val['formobj']);
                    $id = $idmodel->field('id')->where("projectid=" . $projectid["projectid"])->find();
                    if (!empty($id['id'])) {
                        $list = $this->SaveVersionWord($id['id'], $val['formobj'], false, true);
                    }
                    foreach ($list as $k => $v) {
                        if ($v["is_datatable"]) {
                            $data = array();
                    ...
                    $document->clearTemplateTag();
                    $document->save($filenameUTF8Quan);
                    //放进数组
                    array_push($zipUrls, $filenameUTF8Quan);
                }
                //添加进压缩包
                foreach ($zipUrls as $val) {
            ...
            header('Content-disposition: attachment; filename=' . basename($filenameZip)); //文件名
            header("Content-Type: application/zip"); //zip格式的
            header("Content-Transfer-Encoding: binary"); //告诉浏览器，这是二进制文件
            header('Content-Length: ' . filesize($filenameZip)); //告诉浏览器，文件大小
...
// 		$document = $PHPWord->loadTemplate($imgurl);
    ...
    /**
     * @Title: enterToSubmitAudit
     * @Description: todo(套表审核流程弹出意见框)
     * @author wangzhaoxia
     * @date 2015-9-21 下午3:54:15
     * @throws
     */
    function enterToSubmitAuditProcess()
    {
        $type = $_REQUEST["type"];
        $module = $_REQUEST["module"];

        $form = $module . '_auditEdit';
        $this->assign('form', $form);
        $this->display("Public:enterToSubmitAuditProcess");
    }
 ...
 */
     ...
     * @thorws
     */
    function Model2Table()
    {
        $data = $_POST ? $_POST : $_GET;
        if (!empty($data['TitleModel']) || $data['projectid'] !== null) {
            // 一、获取表
            foreach ($data['TitleModel'] as $k => $v) {
                $v = strlen($v) >= 25 ? substr($v, 0, -15) : str_replace('_', '', $v);
                $where = array('modelname' => $v, 'status' => 1);
                $Table[] = M('mis_dynamic_database_mas')->where($where)->select();
            }
            // 二、获取表单id,iframe的bi地址，表名,要存储的字段
            foreach ($Table as $key => $value) {
                foreach ($value as $kt => $vt) {
                    $map = array('formid' => $vt['formid'], 'status' => 1);
                    $rows = M('mis_value_edit_rules')->where($map)->select();
                    if ($rows !== null && !empty($rows)) {
                        foreach ($rows as $i => $j) {
                            $formid = $j['formid'];
                            $savefiledstring = $j['act_obj'];
                            $savefiledstringtotle = strlen($savefiledstring);
                            $savefiledstroposstring = strpos("$savefiledstring", ',');
                            $savefiledstrstr = substr($savefiledstring, $savefiledstroposstring);
                            $savefiledstrlast = substr(substr($savefiledstrstr, 2), 0, -2);
                            $iframe_adress = $j['jg_content'];
                            $strleniframeadress = strlen($iframe_adress);
                            if (strpos($iframe_adress, 'projectid')) {
                                $tpiframeadress = strpos("$iframe_adress", "projectid");
                                $iframe_adress = substr($iframe_adress, -$strleniframeadress, $tpiframeadress) . "projectid={$data['projectid']}";
                            } else if (strpos($iframe_adress, 'abc')) {
                                $tpiframeadress = strpos("$iframe_adress", "abc");
                                $iframe_adress = substr($iframe_adress, -$strleniframeadress, $tpiframeadress) . "abc={$data['projectid']}";
                            }
                            $iframeadress = substr(substr($iframe_adress, 1), 5);
                            $iframe[] = array(
                                'form_id' => $formid, 'table_name' => $vt['tablename'],
                                'iframe_adress' => $iframeadress, 'savefiled' => $savefiledstrlast
                            );
                        }
                    }
                }
            ...
            // 三、处理值，空值，重复相同值等问题
            // [表单id,iframe的bi地址，表名,存储字段,projectid] =>  重装Arr
            foreach ($iframe as $kk => $vv) {
                if ($vv['form_id'] !== null && $vv['iframe_adress'] !== null && $vv['table_name'] !== null) {
                    $heightmap = array('fieldname' => $vv['savefiled'], 'formid' => $vv['form_id']);
                    $height = M("mis_dynamic_form_propery")->where($heightmap)->select();
                    $biData[] = array(
                        'formid' => $vv['form_id'],
                        'iframeadress' => $vv['iframe_adress'],
                        'tablename' => $vv['table_name'],
                        'savefiled' => $vv['savefiled'],
                        'projectid' => $data['projectid'] ? $data['projectid'] : " ",
                        'height' => $height[0]['uploadnum'] ? $height[0]['uploadnum'] : 1300
                    );
                }
            }
            // 用户信息
            $user_info = array(
                ...
                'password' => $data['passwords'] ? $data['passwords'] : "hndgs201804"
            );
            // array_push($biData,$user_info);
        } else {
        }
        // 循环截图开始，调取参数
        $wherestatus = array('projectid' => $data['projectid']);
        // 判断单据状态，只有在终审后才能进行截图。
        $statusdata = M('mis_auto_dbfce')->where($wherestatus)->field('operateid,auditState')->select();
        if ($statusdata[0]['operateid'] == 1 && $statusdata[0]['auditState'] == 3) {
            echo "截图!";
            $this->CuptBIImages($biData, $user_info);
        }
        echo "不截图!";
    }
     ...
     * @Date:15:20
     * @thorws
     * @param string $data BI保存路径
     * @param array $userinfo BI数据
     * @param string $src bi地址
     * @param string $dir BI保存路径
     * @param string $imgType 图片格式
     * @return string
     */
    function CuptBIImages($data, $userinfo, $src = '', $dir = "", $imgType = "")
    {
        if (empty($data) || empty($userinfo)) {
            return false;
        }
        //$username = $userinfo['username'];
        //$password = $userinfo['password'];
        $username = "admin";
        $password = "hndgs201888";
        // 截图参数遍历
        foreach ($data as $keys => $values) {
            $iframeadress = $values['iframeadress'] ? $values['iframeadress'] : $_REQUEST['src'];
            $adressurl = str_replace("&amp;", "&", $iframeadress);
            $biadressurl = str_replace("1.62.154.90:37799", "192.168.10.42:37799", $adressurl);
            $biheight = $values['height'];
            //dump($biadressurl);
            $html =
                <<<html
<html>   
<head>  
	...
	jQuery.ajax({
	     url:"http://192.168.10.42:37799/WebReport/ReportServer?op=fs_load&cmd=sso",//单点登录的报表服务器
	     dataType:"jsonp",
	     ...
	     success:function(data) {
	            ...
	            alert("success");
	            document.write("<iframe "+"src="+'"'+"$biadressurl"+'"'+" width="+"100%"+" height="+"100%"+"/>");
...
html;
            ...
            // 开始生成具体的html文件模板
            // html文件存放目录
            $htmldir = "/home/project/Admin/BIHTML/" . $time . "/";
            if (!file_exists($htmldir)) {
                mkdir($htmldir, 0777, true);
            }
            file_put_contents($htmldir . $keys . "_" . $date . ".html", $html);
            $biurladress = $htmldir . $keys . "_" . $date . ".html";
            if (empty($biurladress)) {
                return "URL为空！...";
            }
            if (empty($dir)) {
                $imageDir = '/home/project/Admin/BIIMG/' . $time;
            } else {
                $imageDir = $dir;
            }
            if (empty($imgType)) {
            ...
            $cuptimagesjs = '/home/jietu/cuptImage.js';
            if (!file_exists($imageDir)) {
                mkdir($imageDir, 0777, true);
            }
            $images_name = $date . "." . $type;
            $image_save_path = $imageDir . '/' . $images_name;
            $phantomjs = "/usr/local/phantomjs/phantomjs/bin/phantomjs";
            $commod = "$phantomjs $cuptimagesjs '$biurladress' $image_save_path $biheight ";
            echo "Loading：" . $images_name . "Cupt Images now..." . "\n";
            $result = array();
            if (exec($commod)) {
                $image_adress = str_replace("/home/project", "1.62.154.90:9010", $image_save_path);
                $result['images_name'] = $images_name;
                $result['image_save_path'] = $image_save_path;
                $result['image_adress'] = $image_adress;
                $result['time'] = time();
                $result['message'] = '截图成功！';
                $result['code'] = 'SUCCESS';
                // 截图成功后删除html文件释放空间
                //if(!unlink($url)){echo "Error deleting $url";}
                //else{echo "Deleted".' '.$url;}
            } else {
                $result['message'] = '其他错误!';
                $result['code'] = 'ERROR';
            }

            $src_save_img_adress = substr($image_adress, 16);
            $src_save_img_titleandalt = substr($image_save_path, -14);
            $src_save_img_data = $src_save_img_data = "&lt;p&gt;&lt;br/&gt;&lt;/p&gt;&lt;p&gt;&lt;p&gt;&lt;img src=&quot;"
                . $src_save_img_adress . "&quot; title=&quot;" . $src_save_img_titleandalt . "&quot; alt=&quot;"
                . $src_save_img_titleandalt . "&quot;/&gt;&lt;/p&gt;&lt;/p&gt;";

//            $src_save_img_data = "<p><br/></p><p><p><img src=".'"'.$src_save_img_adress.'"'." "."title=".'"'.$src_save_img_titleandalt.'"'
//                ." "."alt=".'"'.$src_save_img_titleandalt.'"'."/></p></p>";
            dump($values['tablename']);
            dump($values['savefiled']);
            echo "</br>";
            $imgwhere = array();
            $imgdata = array();
            $imgwhere['projectid'] = $values['projectid'];
            $imgdata[$values['savefiled']] = $src_save_img_data;
            // 开始将图片存库
            M($values['tablename'])->query("SET AUTOCOMMIT=0");
            M($values['tablename'])->query("BEGIN");
            $imgresult = M($values['tablename'])->where($imgwhere)->save($imgdata);
            M($values['tablename'])->query("COMMIT");
            M($values['tablename'])->query("END");
            M($values['tablename'])->query("SET AUTOCOMMIT=1");
            //dump(M($values['tablename'])->getLastSql());
            dump($imgresult);
            //exit;
        }
        exit;
