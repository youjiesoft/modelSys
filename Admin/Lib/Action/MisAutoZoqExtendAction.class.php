<?php
/**
 * @Title: MisAutoZoqAction
 * @Package package_name
 * @Description: todo(动态表单_扩展类。本类为用户代码注入入口，系统一旦生成将不再重复生成。 * 						但当用户选为组合表单方案后会更新该文件，请做好备份)
 * @author 管理员
 * @company 重庆特米洛科技有限公司
 * @copyright 本文件归属于重庆特米洛科技有限公司
 * @date 2015-03-20 10:07:57
 * @version V1.0
*/
class MisAutoZoqExtendAction extends CommonAction {
	...
	*/
	function _extend_after_insert($id){
		...
		if($vlist===false){
			logs($Vphsubmodel->getLastSql() , 'MisAutoZoq_after_insert_'.date('Y-m-d-H' , time()) ,'',__CLASS__,__FUNCTION__,__METHOD__);
	...
	*/
	...
	function rebuildSetting(){
		$setting=array();
		$setting['hiddens']="<input type=hidden value=MisAutoZoq name=jumpaction />"
