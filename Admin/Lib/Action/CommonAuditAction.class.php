...
// 			}
    ...
    public function auditView() {
        ...
        if(false === $vo){
            ...
            echo "<script>console.log(' $model->getDbError()');</script>";
        }
        $name = $this->getActionName();
        $remindMap=array();
        ...
        if($id){
            ...
            if($vo['flowtype'] == 3 && $vo['isauditmodel'] && $vo['issourcemodel']){
                ...
                if($childrenvo['zitableid']){
                    ...
                    $dataArr=$MisSystemDataRoamingModel->main(1,$vo['issourcemodel'],$issourceid,4,$vo ['isauditmodel']);
                    $targetvo=array();
                    ...
                    if($targetvo){
                        //查询目标是否存在数据
                        $targetModel = D($vo ['isauditmodel']);
                        $targetid = $targetModel->where($targetvo)->getField("id");
                    }
                    if($targetid){
                        $process_relation_formDao->startTrans();
                        //找到原始流程的对象及ID
                        $map = array();
                        $map['tablename'] = $name; //子流程模型名称
                        $map['tableid'] = $id;  //子流程id
                        ...
                        M('process_relation_children')->where($map)->setField("zitableid",$targetid);
                        //找到了已存在的任务
                        $process_relation_formDao->where("id = {$vo['id']}")->setField("isaudittableid",$targetid);
                        //先清除当前数据的状态
                        //存在子流程
                        $targetMap = array();
                        $targetMap['id'] = $targetid;
                        $targetMap['operateid'] = 0;
                        $targetMap['auditState'] = 0;
                        $targetResult = $targetModel->save($targetMap);
                        
                        $process_relation_formDao->commit();
                        $data ['url'] = __APP__ . "/" . $vo ['isauditmodel'] . "/edit/id/{$targetid}/is_special/1/auditZhuLicModel/".$name."/auditZhuLicId/".$id."/auditFlowTuiTablename/".$vo['issourcemodel']."/auditFlowTuiTableid/".$issourceid."/projectid/".$projectid."/projectworkid/".$projectworkid;
                        $data ['rel'] = $vo ['isauditmodel'] . "edit";
                        $data ['title'] = $nodename . "_修改";
                    }else{
    ...
    function startprocess($isrelation=false) {
        ...
        if($flowid){
            ...
            //增加关于子流程的主流程被打回，然后重启子流程时，子流程不需要在走流程。启动就完成；
            $is_special = $_POST['is_special'];
            if($pcarr && $is_special!=1){
                /*
				 * 插入新的流程表单表
				 */
        ...
        if(!$isrelation){
            ...
            $personnum = $resultform[0]['curaudituser'];
            $personArray = explode(",",$personnum);
            $rename = array();
            foreach ($resultperson as $key=>$val){
                foreach ($personArray as $va){
                    if($va == $val['userid']){
                        array_push($rename,$val['name']);
                    }
                }
            }
            $personName = implode(",",$rename);
            $this->success('单据启动成功,下一阶段处理人是->'.$personName);
        }else{
            return true;
        }
    }
            ...
            if($infovo['istuihui']){
                ...
                if($newinfo){
                    foreach($newinfo as $key=>$val){
                        ...
                        //打回流程，判断是否存在子流程，如果存在子流程，并且子流程数据已完成，则进行修改子流程数据状态
                        if($val['flowtype'] == 3 && $val['isauditmodel'] && $val['issourcemodel'] && $val['isaudittableid']){
                        	//存在子流程
                        	$targetModel = D($val['isauditmodel']);
                        	$targetMap = array();
                        	$targetMap['id'] = $val['isaudittableid'];
                        	$targetMap['operateid'] = 0;
                        	$targetMap['auditState'] = 0;
                        	$targetResult = $targetModel->save($targetMap);
    ...
    public function auditProcess($step=false,$douserid=0){
        ...
        if($step==false){
            ...
            $resultperson = $person_info->field("name,userid")->select();
            $pwhere = array();
            $pwhere['tableid'] = $masid;
            $where['tablename'] = $name;
            $where['doing'] = 1;
            $where['flowtype'] =array('gt',1);
            $where['auditstate'] = array('eq',0);
            $resultform = $process_relation_formmodel->field('curaudituser,name')->where($where)->select();
            $personnum = $resultform[0]['curaudituser'];
            $personArray = explode(",",$personnum);
            $rename = array();
            foreach ($resultperson as $key=>$val){
