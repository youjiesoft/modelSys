...
//                  }, $template);
            ...
            if ($pageOperate == 2) {
                foreach ($fieldsinfo as $k => $data) {
                    ...
                    if($data [$this->publicProperty ['catalog'] ['name']] == "iframe"){
                        $controll .= $this->cellForOprate($data, 'add', $defaultControlTplArr[$k],$pageCategory);
                    }else{
     ...
     * @throws
     */
    private function cellForOprate($data, $type = 'add', $inTemplate,$pageCategory)
    {
        if (!is_array($data)) {
            $msg = '组件配置数据为空！！';
            throw new NullDataExcetion ($msg);
...
//              $data[$property ['cropfreed'] ['name']]
     ...
     *
     */
    private function getControl($controllProperty, $isedit = false, $isreturnvalue = false, $special = true,$pageCategory)
    {
        ...
        switch ($controllProperty ['catalog']) {
            ...
            case 'datatable' :
                if($controllProperty['istwodimensional']==0){
                    $isaddrowimport = $controllProperty['isaddrowimport'];//是否显示新增行和导入按钮判断依据
                    $viewval = '';
                    $titles
                        = <<<EOF
...
EOF;
                    ...
                    $controlNameRand = $controlName . rand_string();
                    ...
                    usort(
                        $innerTableFieldInfoObj, function ($a, $b) {
                        $al = $a ['fieldsort'];
                        $bl = $b ['fieldsort'];
                        if ($al == $bl)
                            return 0;
                    ...
                    $controlNameRandd = $controlNameRand;
                    $filedList = "";
                    $filedList .= "<th style='width:40px;' template_key=\"serial\"  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1'>";
                    $filedList .= "             序号";
                    $filedList .= "         </th>";
                    $count     = 0;
                    $nameKey   = '';
                        ...
                        if ($val->fieldshow == 1) {
                            ...
                            switch ($val->fieldshowtype) {
                                case 'text':
                                    ...
                                    // @todo
                                    $calculateString = preg_replace_callback(
                                        '/(\[(.*?)\])+/', function ($callbackParame) use ($FormulArr) {
...
EOF;
                                    ...
                                    $filedList .= "\r\n{$is_stats} {$editStatusConfig}  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1'  template_class=\" $untilsCls $curCls \" {$nameKey} >"; // unitlpase
                                    $filedList .= "\r\n{$val->fieldtitle}";
                                    $filedList .= "\r\n</th>";
                                    // logs($filedList);
                                    break;
                                case 'select' :
                                    ...
                                    * template_class="code"
                                    ...
                                    $filedList .= "\r\n         <th {$bindGroup} style=\"width:{$val->colwidth}px;min-width:{$val->colwidth}px;min-width:{$val->colwidth}px;\" template_key='{$template_key}' template_name=\"{$namses}[{$val->fieldname}]\"  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' {$dateOrgConfig}";
                                ...
                                case 'date' :
                                    ...
                                    $filedList .= "\r\n         template_data='{\"format\":\"{$format}\"}'  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' {$editStatusConfig}";
                                    $filedList .= "\r\n         template_class=\" $curCls \"  {$nameKey}>";
                                    $filedList .= "\r\n             {$val->fieldtitle}";
                                    $filedList .= "\r\n         </th>";
                                    $showandeditfieldlist
...
EOF;
                                ...
                                case 'uploadfilenew' :
                                    ...
                                    $filedList      .= "\r\n            template_data='{\"url\":\"__URL__/DT_uploadnew\"}'  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' {$editStatusConfig} ";
                            ...
                            <a
                            title="附件管理"
                            <if condition="getdtcount(\$vo['id'],'{$tableModelname}',\$item['id'],'{$val->fieldname}') eq 0">class="dt_upload_zero"<else/>class="tml_task_btn"</if>
...
EOF;
                                ...
                                case 'lookup' :
                                    ...
                                    $filedList .= "\r\n         <th style=\"width:{$val->colwidth}px;min-width:{$val->colwidth}px;min-width:{$val->colwidth}px;\"  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' template_key=\"lookup\" \r\n ";
                                    $filedList .= "\r\n             template_name=\"\" \r\n ";
...
EOF;
                    ...
                    $datatableID                  = 'dt_' . $controlNameRand . '_' . $tableoprate;
                    ...
                    if ($orgTag) {
                        ...
                        $twodimensional        = $controllProperty['istwodimensional'] == 1?istwodimensional:noistwodimensional;
                        $nval=json_encode(array(
                            'istwodimensional'=>$controllProperty [$property ['istwodimensional'] ['name']],
                            'istwodimensionalshow'=>$controllProperty [$property ['istwodimensionalshow'] ['name']],
                            'tableName'=>$tableName,
                            'tableID'=>$datatableID,
                            'namses'=>$namses,
                            'innerTableFieldInfoObj'=>$innerTableFieldInfoObj,
                        ...
                        <input style="display:none;" type="text" value="datatableinsert" nval='{$nval}' tid="{$propertyid}" otag="{$orgTag}" class="{$orgTag}.datatable_data" dtrelation='{$relation}' dtkey="{$relationDataTableName}" dtid="{$datatableID}" callback="{$dataTableCallBackFunc}" />
EOF;
                    }
                    $datatableLookupInsert=<<<EOF
                        <input type="hidden" name="istwodimensional[{$controlName}]" value="{$controllProperty['istwodimensional']}"/>
EOF;
                    ...
                    $temp
                        = <<<EOF
                                                ...
                                                {$datatableLookupInsertDataTag}
                                                {$datatableLookupInsert}
                                                {$titles}
                                                <table class="into_table_new nbm_data_table {$twodimensional}" distinguish="{$orgTag}" isaddrowimport="{$isaddrowimport}" table_data='{"formModel":"{$this->nodeName}","datatableModel":"{$dataTableModelName}","importUrl":"__APP__/MisImportExcel/misimportexceladd"}' id="{$datatableID}" table_type="{$tableoprate}">
...
EOF;
                                        ...
                                        $controlNameRandd = $controlNameRand;
                                        $filedList .= "\r\n<th {$bindhz} style=\"width:{$val->colwidth}px;min-width:{$val->colwidth}px;min-width:{$val->colwidth}px;\" template_key=\"input\" template_name=\"{$namses}[{$val->fieldname}]\" {$tempData} {$stats_num}";
                                        $filedList .= "\r\n{$is_stats} {$editStatusConfig} template_class=\"$untilsCls $curCls\"  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' {$nameKey}>"; // unitlpase
                                        $filedList .= "\r\n{$val->fieldtitle}";
                                        $filedList .= "\r\n</th>";
                                        // logs($filedList);
                                        break;
                                    ...
                                    case 'date' :
                                        ...
                                        $filedList .= "\r\n         <th style=\"width:{$val->colwidth}px;min-width:{$val->colwidth}px;min-width:{$val->colwidth}px;\" template_key=\"date\" template_name=\"{$namses}[{$val->fieldname}]\"  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' {$dateOrgConfig} ";
...
EOF;
                                    ...
                                    case 'lookup' :
                                        ...
                                        $lookupvaluechange    = '';
                                        if ($valField && $showFiled && $modelName) {
                                            $lookupvaluechange = "{\$collection[0].{$val->fieldname}|getFieldBy='{$valField}','{$showFiled}','{$modelName}'}";
                                        } else {
                                            $lookupvaluechange = "{\$collection[0].{$val->fieldname}}";
                                        }
                                        if ($editStatusConfig) {
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<input type=\"text\" callback=\"\" class=\"list_public_input list_lookup_input readonly org{\$key}{$val->fieldname}.{$showFiled} \" readonly=\"readonly\" value=\"{$lookupvaluechange}\">";
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<input type=\"hidden\" class=\"org{\$key}{$val->fieldname}.{$valField} $curCls\" disabled=\"disabled\" name=\"#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]\" value=\"{\$item.{$val->fieldname}}\">";
                        ...
                        foreach ($innerTableFieldInfoObj as $key => $val) {
                            ...
                            if ($val->fieldshow == 1) {
                                ...
                                switch ($val->fieldshowtype) {
                                    ...
                                    <div class="list_group_lay" style="display: none;">
                                        <div class="list_public_lay">
                                            <input type="text" $y_num $dt_readonly class="$class_readonly $orgshowfield {$edit_calculate_cls}  {$edit_count_cls} {$curCls} list_unit_input list_public_input" {$edit_count_str} disabled="disabled" name="#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]" value="{\$collection[0][{$val->fieldname}]|unitExchange=###,$unit,$untils,2}">
                                            <span class="list_icon_elm_unit" title="{$config['untilschar']}">{$config['untilschar']}
                                            </span>
                                        </div>
                                    </div>
                                    <span class="datatable_show_val">{\$collection[0][{$val->fieldname}]|unitExchange=###,$unit,$untils,3}</span>
EOF;
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t</td>";
                                        } else {
                                            $showandeditfieldlist
                                            ...
                                            <td>
                                            <div class="list_group_lay" style="display: none;">
                                            <input type="text" $dt_readonly $y_num class="list_input $class_readonly {$edit_calculate_cls}  {$untilsCls} {$orgshowfield} {$edit_count_cls} {$curCls}" disabled="disabled" name="#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]" {$untils} value="{\$collection[0][{$val->fieldname}]}" />
                                                </div>
                                                <span class="datatable_show_val">{\$collection[0][{$val->fieldname}]}</span>
                                                </td>
EOF;
                                        }
                                        ...
                                        * template_class="code"
                                        ...
                                        if ($config ['datasouce'] == 0) {
                                            ...
                                            if ($selectInc) {
                                                ...
                                                $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<select class=\"list_select2 $class_readonly {$orgshowfield} $curCls \" data-placeholder=\"没有可用数据\"";
                                                $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t name=\"#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]\">";
                                                $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t{:getControllbyHtml('selectlist',array('type'=>'select','names'=>\"datatable[\$key][{$controlTag}][{$val->fieldname}]\",'key'=>'{$selectInc}','selected'=>\$collection[0]['{$val->fieldname}']))}";
                                                $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t</select></div><span class='datatable_show_val'>{:getControllbyHtml('selectlist',array('type'=>'select','names'=>\"datatable[\$key][{$controlTag}][{$val->fieldname}]\",'key'=>'{$selectInc}','selected'=>\$collection[0]['{$val->fieldname}'],'showtype'=>1))}</span>";
                                                $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t</td>";
                                            }
                                        } else {
                                            // 数据源为 table
                                            if ($lookupConfig [0] && $lookupConfig [1] && $lookupConfig [2]) {
                                                if ($lookupConfig ['treeparentfield']) {
                                                    ...
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t<td>";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<div style='display: none;' class='list_group_lay'>";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t{:W('ShowSelectDT',array(\$collection[0]['{$val->fieldname}'], array(' $curCls ',array('type'=>'table',array('readonly'=>'1','targevent'=>'','actionName'=>'{$this->nodeName}','names'=>\"#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]\",'defaultcheckitem'=>'','defaultval'=>'','defaulttext'=>'','table'=>'{$lookupConfig[0]}', 'id'=>'{$lookupConfig[2]}','name'=>'{$lookupConfig[1]}','conditions'=>'','parentid'=>'{$lookupConfig['treeparentfield']}','mulit'=>'{$lookupConfig['mulit']}','isnextend'=>'{$lookupConfig['isnextend']}','treeheight'=>'{$lookupConfig['treeheight']}','treewidth'=>'{$lookupConfig['treewidth']}','key'=>'','isedit'=>'{$int_readonly}','showtype'=>'0','namesappend'=>'{$val->fieldname}#index#')))))}";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t</div><span class='datatable_show_val'>{:getControllbyHtml('table',array('type'=>'select','names'=>\"datatable[\$key][{$controlTag}][{$val->fieldname}]\",'table'=>{$lookupConfig[0]},'id'=>'{$lookupConfig[2]}','name'=>'{$lookupConfig[1]}','selected'=>\$collection[0]['{$val->fieldname}'],'showtype'=>1))}</span>";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t</td>";
                                                    $chkStyle             = "radio";
                                                    if ($lookupConfig ["mulit"] == 1) {
                                                        $chkStyle = "checkbox";
                                                    ...
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<div style='display: none;' class='list_group_lay'><select class=\"list_select2 $class_readonly {$orgshowfield} $curCls \" data-placeholder=\"没有可用数据\"";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t name=\"#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]\">";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t{:getControllbyHtml('table',array('type'=>'select','names'=>\"datatable[\$key][{$controlTag}][{$val->fieldname}]\",'table'=>'{$lookupConfig[0]}','id'=>'{$lookupConfig[2]}','name'=>'{$lookupConfig[1]}','selected'=>\$collection[0]['{$val->fieldname}']))}";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t</select></div><span class='datatable_show_val'>{:getControllbyHtml('table',array('type'=>'select','names'=>\"datatable[\$key][{$controlTag}][{$val->fieldname}]\",'table'=>'{$lookupConfig[0]}','id'=>'{$lookupConfig[2]}','name'=>'{$lookupConfig[1]}','selected'=>\$collection[0]['{$val->fieldname}'],'showtype'=>1))}</span>";
                                                    $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t</td>";
                                                }
                                            }
                                        }
                                        ...
                                        $filedList .= "\r\n         <th {$bindGroup} style=\"width:{$val->colwidth}px;min-width:{$val->colwidth}px;min-width:{$val->colwidth}px;\" template_key='{$template_key}'  onclick=\"neiqianpaixu('dt_$controlNameRandd', this)\" mode='1' template_name=\"{$namses}[{$val->fieldname}]\"  {$dateOrgConfig}";
                                        $filedList .= "\r\n         template_data='{$template_data}' {$editStatusConfig}";
                                        $filedList .= "\r\n         template_html='{$getControllbyHtmlStr}'";
                                        $filedList .= "\r\n         template_class=\" $curCls \"  {$nameKey}>";
                                        $filedList .= "\r\n             {$val->fieldtitle}";
                                    ...
                                    case 'date' :
                                        ...
                                        $showandeditfieldlist
                                            .= <<<EOF
                                                ...
                                                <td>
                                                <div class="list_group_lay" style='display: none;'>
                                                <input type="text" {$dt_readonly} disabled="disabled" name="#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]" class="Wdate js-wdate list_input {$orgshowfield} {$class_readonly}  $curCls " format="{dateFmt:'{$format}'}" value="{\$collection[0].{$val->fieldname}|transTime{$format1}}">
                                                </div>
                                                <span class='datatable_show_val'>{\$collection[0].{$val->fieldname}|transTime{$format1}}</span>
                                                </td>
EOF;
                                        break;
                                    case 'uploadfilenew' :
...
EOF;
                                    ...
                                    case 'lookup' :
                                        ...
                                        if ($editStatusConfig) {
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<input type=\"text\" callback=\"\" class=\"list_public_input list_lookup_input readonly org{\$key}{$val->fieldname}.{$showFiled} \" readonly=\"readonly\" value=\"{$lookupvaluechange}\">";
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<input type=\"hidden\" class=\"org{\$key}{$val->fieldname}.{$valField} $curCls\" disabled=\"disabled\" name=\"#hide#datatable[#index#][{$controlTag}][{$val->fieldname}]\" value=\"{\$collection[0].{$val->fieldname}}\">";
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<a class=\"list_icon_elm list_mid_icon_elm icon-plus\"></a>";
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t<a title=\"清空信息\" class=\"list_icon_elm icon-trash\" href=\"javascript:void(0);\"></a>";
                                            $showandeditfieldlist .= "\r\n\t\t\t\t\t\t\t\t\t\t\t</div></div>";
                                        } else {
                    ...
                    if($pageCategory!=='add'){
                        $tableErweiTitle
                            =<<<EOF
                            <volist name="yum{$controllProperty['tablename']}_sub_{$controllProperty['fields']}" id="item">
										<th style="width:px;min-width:px;min-width:px;" template_key="input"  template_name="datatable[#index#][{$controllProperty['fields']}][{\$item[name]}_{\$item[code]}]"

											template_class="  "  template_controll={\$item[name]} class="delete">
...
EOF;
                    ...
                    if($pageCategory!=='add'){
                        $tableErweiContent
                            =<<<EOF
                            <volist name="collection" id="val">
											<td>
												<div class="list_group_lay" style="display: none;">
													<input type="text"   class="list_input       " disabled="disabled" name="#hide#datatable[#index#][{$controllProperty['fields']}][{\$val[zhibiaomingchen]}_{\$val[zhibiaobianma]}_{\$val[rowcontens]}]"  value="{\$val[jine]}" />
												</div>
												<span class="datatable_show_val">{\$val[jine]}</span>
											</td>
											<input type="hidden" name="#hide#datatable[#index#][{$controllProperty['fields']}][id]" value="{\$val[id]}" />
										</volist>
EOF;
                ...
                <td {$oprateItem}>
                        <input type="hidden" name="#hide#datatable[#index#][{$controlTag}][id]" value="{\$item.id}" />
                        <button title="删除" type="button" class="into_table_new_trash_tr into_table_btn itb_del" del_url="__URL__/delsubinfo/delmodel/{$dataTableModelName}/erwei/1" del_id="{\$item.id}" del_table="{$tableName}"  nbmonclick="del_sub_info('$this->nodeName','{$tableName}',{\$item.id},this)">
                        <span class="icon-remove"></span></button>
                        <button post_id="{\$item.id}" post_url="__URL__/onesave" post_id="{\$item.id}" post_table="{$tableName}" rel_type="edit" class="save_row_btn" type="button" title="编辑"><span class="icon-pencil"></span></button>
                    </td>
EOF;
                ...
                <td {$oprateItem}>
                        <input type="hidden" name="#hide#datatable[#index#][{$controlTag}][id]" value="{\$collection[0].id}" />
                        <button title="删除" type="button" class="into_table_new_trash_tr into_table_btn itb_del" del_url="__URL__/delsubinfo/delmodel/{$dataTableModelName}/erwei/1" del_id="{\$collection[0].id}" del_table="{$tableName}"  nbmonclick="del_sub_info('$this->nodeName','{$tableName}',{\$collection[0].id},this)">
                        <span class="icon-remove"></span></button>
                        <button post_id="{\$collection[0].id}" post_url="__URL__/onesave" post_id="{\$collection[0].id}" post_table="{$tableName}" rel_type="edit" class="save_row_btn" type="button" title="编辑"><span class="icon-pencil"></span></button>
                    </td>
EOF;
                    }
                    if ($isreturnvalue) {
...
EOF;
                    ...
                    $datatableLookupInsert=<<<EOF
                        <input type="hidden" name="shanchsj[{$controlName}]" value="{$dataTableModelName}"/>
                        <input type="hidden" name="istwodimensional[{$controlName}]" value="{$controllProperty['istwodimensional']}"/>
EOF;
...
EOF;
                        ...
                        $temp
                            .= <<<EOF
                                                ...
                                                {~\$key=0;}
                                                <volist name="rowcontens{$controllProperty['tablename']}_sub_{$controllProperty['fields']}" id="collection">
                                                <tr>
                                                <td>
                                                <span class="serial_number">
                                                {\$key+1}
