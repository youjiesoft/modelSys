...
//         $this->writeover($filename,"\$aryRule = ".$this->pw_var_export($data).";\n",true);
    ...
    public function getRoleTree($rel,$url='__URL__/index',$map2=array(),$modelname=false,$target='ajax',$title=''){
    	...
    	$data = $node->where($map)->order('sort asc')->field("id,pid,name,level,isprojectwork,type,group_id,title,toolshow,showmenu")->select();
   		if($map2){
   			$map['level']=array('eq',3);
   			$map=array_merge($map,$map2);
   			$data2 = $node->where($map)->field("id,pid,name,level,isprojectwork,type,group_id,title,toolshow,showmenu")->select();
   			$cond = $data2;
   			$top=array();
   			foreach ($data as $k=>$v){
