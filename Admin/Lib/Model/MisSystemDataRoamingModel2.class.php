...
// 			}
			//获取来源数据并分流
			$mainObject=array();//logs($roamMasVal,"gml");
			//数据源对象
			$tableObj=json_decode($roamMasVal['strelation'],true);//logs($tableObj,"gml");
            //print_r($tableObj);
			//循环对象
			...
			foreach($tableObj as $key => $val){
				...
				switch($sourceObjType){
				    ...
				    //内嵌表
					case 2:logs("333","gml");
						$object=self::dataRoamSublistObject($sourceObjTable,$pkValue);logs($object,"gml");
						break;
					//视图
					case 3:
...
// 						}
			...
			//$sourcetype=4;
			//logs($mainObject,"gml");logs("222333222","gml");
			switch($sourcetype){
				//漫游生单，这个是不可能有循环的
				case 4:
	...
	static private function dataRoamMasCheck($isbindsettable,$sourcemodel,$pkValue,$sourcetype,$targetmodel,$roamMasid){
		switch($sourcetype){
			case 4:
				...
				$map['status']=1;
				$masResult=$masModel->where($map)->select();//logs($masResult,"gml");
//  				print_r($masModel->getLastSql());				
	...
	static private function dataRoamSourceObject($tableObj,$pkValue,$rules,$rulesType){logs("111","gml");
		...
		}logs(M($tableObj)->getLastSql(),"gml");
 		//        print_r($object);
		...
		if(!$bool){
			switch ($objInfo['targetobjtype']){
				case 1:
					//主表
					//新增时要更新单号规则
                    if ($roamMasVal["sourcemodel"]=='MisSalesMyProject'&& $_POST['xinzeng']==1) {
						$scnModel=D("SystemConfigNumber");
                        return $ordernoInfo=$scnModel->getOrderno($objInfo['targettable'],$roamMasVal['targetmodel']);
                    }
					$scnModel=D("SystemConfigNumber");
					$ordernoInfo=$scnModel->getOrderno($objInfo['targettable'],$roamMasVal['targetmodel']);
//					$kim=M('mis_auto_kimpu');
