...
EOF;
			...
			if($parentid){
				...
				$html=<<<EOF
              <input type="text" readonly="readonly"  id="xifenval" class="{$conboxtreeCls} {$classStyle} input_new" size="18" value="{$nameVal}" data-names="{$fieldName}" data-height="{$treeheight}" data-dialog="{$treedialog}" {$treewidthParame} data-tree="#{$ztreeId}" data-search="true" style="background:white url(/Public/Images/tb3.jpg) no-repeat  right top ;" />
              <input type="hidden" name="{$fieldName}" id="xinfenno" {$dropbackkeyStr} class="{$hiddenRequired}" value="{$IdVal}"/>
