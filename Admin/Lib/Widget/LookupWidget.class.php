...
// EOF;
			...
			$html .="<input type=\"hidden\"  {$valCounterCheck}  class=\"{$required} {$lookupgroup}.{$orgval} {$orgLookuoVal}\" name=\"".$controllProperty[$property['fields']['dbfield']]."\" value=\"".$vo[$controllProperty[$property['fields']['dbfield']]]."\"  id=\"{$lookupgroup}\" />";
			if($readonly){ // 只读情况
				// lookup 触发按钮
				...
				// lookup 触发按钮
				$html .="<a class=\"icon_elm mid_icon_elm icon-plus\" syscondition=\"{$systemconditions}\"  newconditions=\"{$conditions}\"  {$conditionOprateString}  param=\"lookupchoice={$lookupchoice}&newconditions={$view}{$dtcon}\" href=\"__URL__/{$urls}/lookupgroup/{$lookupgroup}\" lookupGroup=\"{$lookupgroup}\" onclick='lookupcheck(this)'></a>";//$title
				// 清空 lookup 按钮
