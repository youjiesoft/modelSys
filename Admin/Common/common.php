...
function getOperateStatus($creatid, $auditstatus, $param = "", $modelname = "", $rel = "", $target = "navTab", $title = "", $width = "", $height = "", $mask = true)
{
    ...
    if (! isset($_SESSION['a'])) {
        if ($_SESSION[strtolower($modelname . '_update')] == 1) { // 判断全部修改权限
            ...
            if ($_SESSION[strtolower($modelname . '_update')] == 2) { // //判断部门及子部门权限
                
                $qx = in_array($creatid, $_SESSION['user_dep_all_child']) ? true : false;
            } else 
                if ($_SESSION[strtolower($modelname . '_update')] == 3) { // 判断部门权限
                    $qx = in_array($creatid, $_SESSION['user_dep_all_self']) ? true : false;
                } else 
                    if ($_SESSION[strtolower($modelname . '_update')] == 4) { // 判断个人权限
...
function getTimeColor($t)
{
    ...
    if (intval($t)) {
        ...
        if ($t2 <= 120) { // 2分钟
            ...
            if ($t2 <= 240 && 120 < $t2) { // 4分钟
                $color = 'd68500';
            } else 
                if ($t2 <= 360 && 240 < $t2) { // 6分钟
                    $color = 'aa6900';
                } else 
                    if ($t2 <= 480 && 360 < $t2) { // 8分钟
                        $color = '835100';
                    } else 
                        if ($t2 <= 600 && 480 < $t2) { // 10分钟
                            $color = '513200';
                        }
    }
    return $color;
...
function getDigits($num = 0, $returndigits = true, $prodid, $unitid)
{
    ...
    if ($returndigits) {
        ...
        if ($unitid) {
            ...
            // 获取单位类型
            $unittype = $model->table('mis_product_unit as mis_product_unit,mis_product_unitexchange as mis_product_unitexchange')
                ->where('mis_product_unit.id = mis_product_unitexchange.subunitid and mis_product_unitexchange.id=' . $units)
                ->getField('mis_product_unit.unittype as unittype');
            if ($unittype == false) {
                $formatDigits = false;
            }
        } else {
            if ($prodid) {
                ...
                $unittype = $model->table('mis_product_unit as mis_product_unit,mis_product_code as mis_product_code')
                    ->where('mis_product_unit.id = mis_product_code.baseunitid and mis_product_code.id=' . $prodid)
                    ->getField('mis_product_unit.unittype as unittype');
                if ($unittype == false) {
                    $formatDigits = false;
                }
 ...
 * @param string $v1            
 * @param array $fun_nam
 *            处理的方法 ex：array('0'=>'trim',1'=>'getFieldBy')
 * @param array $fun_arr
 *            处理的方法对应的参数 ex：array('0'=>array(param1,param2),1'=>array(param1,param2));
 * @param array $extention_data
 *            扩展参数 ex：array('0'=>array(param1,param2),1'=>array(param1,param2));
 * @param bool $out
 *            是否导出
 *            +----------------------------------------------------------
 ...
 *            传入名
 * @param int $type
 *            类型默认为用户授权，1为高级组授权 2为面板模块授权
 * @param int $userid
 *            多组织架构使用 为实现授权管理用户授权--2014-12-26 renlin
 * @return string
 * @author yangxi
...
function getExecutiveCondition($id, $fabuzhuangtai, $zhixingqingkuang, $moxingming, $fabupingtaiid, $type)
{
    ...
    } elseif ($zhixingqingkuang == null || $zhixingqingkuang == '') {} else {
        $title = '查看';
...
function getProductPictureUrl($pcode = "")
{
    if ($pcode) {
        ...
        $map['status'] = 1;
        $map['code'] = $pcode;
        $code = $model->where($map)
            ->field('id,code,typeid')
            ->find();
 ...
 * @param 控制器名称 $m
 *            如果传入了控制器名称，则进入这个控制器进行操作。否则就模型当前调用方法的控制器
 * @param 控制器方法名称 $a
 *            调用的方法名称
 * @param 当前URL的class名称 $class
 *            class代表有什么图标显示
 * @param 弹出的窗口的tabid $rel            
 * @param 当前弹出窗口类型 $target
 *            dialog,navTab
 ...
 * @throws 模板调用方法: {$vo.id|createQRCode=###,'www.laicaoyuan.com','Images/ico.png'}
 */
 ...
 * @param 踢出不需要的value $akickout
 *            数组/字符串
 * @param
 *            判断是否踢出 默认为踢出 否则为包含 $ifkickout boolean
 * @author 杨东
...
function getDataBaseLevelByHtml($sTable, $param)
{
    ...
    $aList = $model->modelShow($sTable, array(
        'key' => 'id',
        'pkey' => 'parentid',
        'fields' => "id,name",
        'conditions' => $sWhere
        ...
        switch ($sType) {
            case "select":
                foreach ($aList as $k => $v) {
                    ...
                    if ($sSelected != null && $sSelected == $v['id'])
                        $str .= ' selected="selected" ';
                    $str .= '>' . $v['name'] . '</option>';
                }
                ;
 ...
 * @param boolean $isFmt
 *            不处理。处理格式后结果变为数组，不处理结果为string
 * @param string $mapType
 *            =1 类型为表时才生效。默认空值，不做处理。1：string类型，及以 《字段 运算符 值》为格式，2：ThinkPHP map类型，$map[字段] = 值，或$map[字段]=array('运算规则',值集合字符串);
...
function getControllbyHtml($datatype, $parame)
{
    ...
    if (! $type)
        return "<error>未知数据来源：{$datatype},当前允许类型：table,selectlist</error>";
    $html = '';
    switch ($type) {
        case "table":
...
EOF;
                    ...
                    case "select":
                        ...
                        if ($showtype == 'yes') {
                            if ($comboxtree == 'yes' && $parentid) {
                                ...
                                $treeSelectselect6Data = $model->modelShow($table, array(
                                    'key' => 'id',
                                    'pkey' => $parentid,
                                ...
                                $treeSelectDataTemp = '';
                                if ($defaulttext) {
                                    array_unshift($treeSelectselect6Data, array(
                                        $id => $defaultval,
                                        $name => $defaulttext,
                                        $parentid => 0,
                                        'nextEnd' => 1
                    ...
                    case "radio":
                        ...
                        if ($showtype == 'yes') {
                            ...
                            foreach ($aList as $k => $v) {
                                ...
                                if (isset($tagSelected) && $tagSelected == $v[$valF])
                                    $html .= ' checked="checked" ';
                                $html .= '/><label class="tmp_label"  for="' . str_replace('=', '', base64_encode($tagName)) . $index . '">' . $v[$textF] . '</label>';
                                $index ++;
                            }
        ...
        case "selectlist":
            ...
            if (! $key)
                return "<error>未知的selectlist.inc key值</error>";
            $list = $list[$key][$key];
                ...
                switch ($tagType) {
                    case "select":
                        if ($showtype == 'yes') {
                            ...
                            foreach ($list as $k => $v) {
                                $html .= '<option value="' . $k . '"';
                                if (isset($tagSelected) && $tagSelected == $k)
                                    $html .= ' selected="selected" ';
                                $html .= '>' . $v . '</option>';
                            }
                            ;
...
function getAreaGroupData()
{
    ...
    if (is_array($catchdata)) {
        ...
        $obj->__construct();
        $obj->modelShow('MisSystemAreas', array(
            'key' => 'id',
            'pkey' => 'parentid',
 ...
 * 
 * @param array $data
 *            标签构成数据 例如：$data=>array('title'=>'类型','id'=>'ckbType','name'=>'type','type'=>'checkbox','value'=>'类型的值')
 * @param boolean $defaultchk            
...
function logs($msg = '', $filename = '', $basepath = '', $class = '', $function = '', $method = '', $contenttype = 'line')
{
    ...
    $quto = '┏━━━━━━━━━━━━━━━━━━' . chr(13) . chr(10) . '┣	IP：' . getip() . chr(13) . chr(10) . '┣	请求对象:' . MODULE_NAME . chr(13) . chr(10) . '┣	UID		:' . $_SESSION[C('USER_AUTH_KEY')] . chr(13) . chr(10) . '┣	NAME	:' . $_SESSION['loginUserName'] . chr(13) . chr(10) . '┣	CLASS	:' . $class . chr(13) . chr(10) . '┣	METHOD	:' . $function . chr(13) . chr(10) . '┣	FUNCTION:' . $method . chr(13) . chr(10) . '┣	Time	:' . Date('Y-m-d H:i:s', time()) . chr(13) . chr(10) . '┣ Note: ' . $msg . chr(13) . chr(10) . '┗━━━━━━━━━━━━━━━━━━' . chr(13) . chr(10);
    
    $line = 'Time ' . Date('Y-m-d H:i:s', time()) . ' : ' . $msg . chr(13) . chr(10);
    ...
    error_log($content, 3, $basepath . '/' . $filename . '.log');
    ...
    switch ($filedtype) {
        case 'varchar':
            if ($infieldtype == "int") {
                ...
                if ($infieldtype == "decimal") {
                    $disabled = $str;
                } else 
                    if ($infieldtype == "smallint") {
                        $disabled = $str;
                    } else 
                        if ($infieldtype == "tinyint") {
                            $disabled = $str;
                        } else 
                            if ($infieldtype == "date") {
                                $disabled = $str;
 ...
 *            指定数据源
 * @param
 *            array		需要组合的字段列表 二维数组，key为当前字段，value为条件上使用的字段名。
 * @author quqiang
...
function upperMoney($ns)
{
    ...
    $ns2 = array_filter(array(
        $ns2[1],
        $ns2[0]
    ));
    $ret = array_merge($ns2, array(
        implode("", _cny_map_unit(str_split($ns1), $grees)),
        ""
    ));
 ...
 * 日期转中文格式
 */
function ubdate($createtime){
    $str_Ystr = "";
    $str_Mstr = "";
    $str_Dstr = "";
    $str = date("Y-m-d",$createtime);
    $str_arr = explode('-',$str);
    $str_Yarr = str_split($str_arr[0]);
    $str_Marr = str_split($str_arr[1]);
    $str_Darr = str_split($str_arr[2]);
    //年转化
    foreach ($str_Yarr as $val){
        switch ($val){
            case 0:$str_Ystr .= '〇';break;
            case 1:$str_Ystr .= '一';break;
            case 2:$str_Ystr .= '二';break;
            case 3:$str_Ystr .= '三';break;
            case 4:$str_Ystr .= '四';break;
            case 5:$str_Ystr .= '五';break;
            case 6:$str_Ystr .= '六';break;
            case 7:$str_Ystr .= '七';break;
            case 8:$str_Ystr .= '八';break;
            case 9:$str_Ystr .= '九';break;
        }
    }
    //月转化
    switch ($str_Marr[0]){
        case 0:$str_Mstr .= '';break;
        case 1:$str_Mstr .= '十';break;
    }
    switch ($str_Marr[1]){
        case 0:$str_Mstr .= '';break;
        case 1:$str_Mstr .= '一';break;
        case 2:$str_Mstr .= '二';break;
    }
    //日转化
    switch ($str_Darr[0]){
        case 0:$str_Dstr .= '';break;
        case 1:$str_Dstr .= '十';break;
        case 2:$str_Dstr .= '二十';break;
        case 3:$str_Dstr .= '三十';break;
    }
    switch ($str_Darr[1]){
        case 0:$str_Dstr .= '';break;
        case 1:$str_Dstr .= '一';break;
        case 2:$str_Dstr .= '二';break;
        case 3:$str_Dstr .= '三';break;
        case 4:$str_Dstr .= '四';break;
        case 5:$str_Dstr .= '五';break;
        case 6:$str_Dstr .= '六';break;
        case 7:$str_Dstr .= '七';break;
        case 8:$str_Dstr .= '八';break;
        case 9:$str_Dstr .= '九';break;
    }
    return $str_Ystr.'年'.$str_Mstr.'月'.$str_Dstr.'日';
}
...
EOF;
            ...
            $MisAutoBindSettableList = $MisAutoBindSettableModel->where($map)
                ->order("inbindsort asc")
                ->select();
            // 查询满足条件的表单类型
                            ...
                            if (! $soureVo) {
                                ...
                                if ($addate) {
                                    ...
                                    if ($resultid) {
                                        ...
                                        $MisDynamicFormProperyList = $MisDynamicFormProperyModel->where($ProperyMap)
                                            ->field("dbname,fieldname,id")
                                            ->select();
            ...
            foreach ($MisAutoBindSettableList as $mkey => $mval) {
                $cstr="taobiaoname='{$mval['inbindaname']}_{$randUniqueTag}' name='{$mval['inbindaname']}_{$randUniqueTag}' ";
                if($i==0){
                    $cstr="";
                } 
                ...
                if (($type == "edit" || $type == "add") && $mval['formshowtype'] != '1') {
                    ...
                    if ($typid >= 1) { // miniindex
                        $html .=  $cstr." href=\"__APP__/{$mval['inbindaname']}/miniindex/func/{$type}/bindtype/{$mval['bindtype']}/bindaname/{$mval['bindaname']}{$mainBindActionCondition}/fieldtype/{$mval['inbindval']}/{$mval['inbindval']}/{$inbindval}/bindrdid/{$vo['id']}{$dataroamingCondition}#{$mval['inbindaname']}_{$randUniqueTag}";
                    } else {
                        if ($mval['inbindid']) { // 修改表单
                            $html .= $cstr." href=\"__APP__/{$mval['inbindaname']}/edit{$mainBindActionCondition}/fieldtype/{$mval['inbindval']}/{$mval['inbindval']}/{$vo[$mval['bindval']]}/bindrdid/{$vo['id']}/bindaname/{$mval['bindaname']}/id/{$mval['inbindid']}#{$mval['inbindaname']}_{$randUniqueTag}";
                        } else { // 新增表单
                            $html .=  $cstr."  href=\"__APP__/{$mval['inbindaname']}/add{$mainBindActionCondition}/bindaname/{$mval['bindaname']}/fieldtype/{$mval['inbindval']}/bindrdid/{$vo['id']}/{$mval['inbindval']}/{$vo[$mval['bindval']]}{$dataroamingCondition}#{$mval['inbindaname']}_{$randUniqueTag}";
                        }
                    }
                    $html .= "\" rel=\"{$mval['inbindaname']}_edit\" class=\"j-ajax\"><span>" . $inbindtitle . "</span></a></li>";
                } else 
                    if ($type == "view" || $mval['formshowtype'] == 1) {
                        $html .= "<li><a id=\"{$mval['inbindaname']}_{$randUniqueTag}\" ";
                        if ($typid >= 1) { // miniindex
                            $html .= $cstr." href=\"__APP__/{$mval['inbindaname']}/miniindex/func/{$type}/bindrdid/{$vo['id']}/minitype/1/viewtype/view/bindtype/{$mval['bindtype']}/bindaname/{$mval['bindaname']}/fieldtype/{$mval['inbindval']}/{$mval['inbindval']}/{$vo[$mval['bindval']]}#{$mval['inbindaname']}_{$randUniqueTag}";
                        } else {
                            $html .= $cstr." href=\"__APP__/{$mval['inbindaname']}/view/bindrdid/{$vo['id']}/bindaname/{$mval['bindaname']}/id/{$mval['inbindid']}#{$mval['inbindaname']}_{$randUniqueTag}";
                        }
            ...
            if (getFieldBy($actionname, "bindaname", "inbindaname", "MisAutoBind")) {
                // 拼装套表js
                $jshtml = <<<EOF
		<script>
		function {$funcitonname}(json) {
...
EOF;
                ...
                if ($type == "edit" || $type == "view" || $iszctpl || $iszhtpl) {
                    ...
                    if (! $iszctpl) { // 组合表单
                        ...
                        // 查询符合条件的表单
                        $MisAutoBindVo = $MisAutoBindModel->where("status=1 and bindaname='{$actionname}'  and bindresult<>'' and typeid=0")
                            ->field("bindresult")
                            ->find();
                        // 过滤掉可能的错误。
                        ...
                        $MisAutoBindSettableList = $MisAutoBindModel->where($bindMap)
                            ->order("inbindsort asc")
                            ->getField("id,inbindaname,bindtype,bindaname,dataroamid,inbindtitle,inbindsort,formshowtype,inbindmap,bindconlistArr");
                    } else {
                        $bindid = $vo['id'];
                        // 查询符合条件的表单
                        $MisAutoBindSettableList = $MisAutoBindModel->where("status=1 and bindaname='{$actionname}'  and typeid=1")
                            ->order("inbindsort asc")
                            ->select();
                    }
                    $MisSystemDataRoamingModel = D("MisSystemDataRoaming");
                    ...
                    foreach ($MisAutoBindSettableList as $key => $val) {
                        ...
                        if ($val['bindtype'] == 0) {
                            ...
                            if (! $MisAutoBindSettableList[$key]['id'] && $type == "edit" && $MisAutoBindSettableList[$key]['formshowtype'] == 0) {
                                ...
                                if (! $MisAutoBindSettableList[$key]['id'] && $type == "add") {
                                    // 为新增页面赋初始值
                    ...
                    foreach ($MisAutoBindSettableList as $makey => $maval) {
                        ...
                        if ($type == "edit" && $maval['formshowtype'] != 1) {
                            if ($typid >= 1) {
                                $html .= $cstr." href=\"__APP__/{$maval['inbindaname']}/miniindex/func/{$type}/bindaname/{$actionname}/bindtype/{$maval['bindtype']}{$mainBindActionCondition}/bindrdid/{$vo['id']}/bindid/{$bindid}/urdataroamid/{$maval['urdataroamid']}\"";
                            } else {
                                unset($tempMap);
                                unset($tempModel);
                                unset($tempData);
                                ...
                                $html .= $cstr." href=\"__APP__/{$maval['inbindaname']}/edit{$mainBindActionCondition}/bindid/{$bindid}/bindrdid/{$vo['id']}/urdataroamid/{$maval['urdataroamid']}/id/" . $idTempValue . "\"";
                            }
                        } else 
                            if ($type == "view" || $maval['formshowtype'] == 1) {
                                if ($typid >= 1) {
                                    $html .= $cstr." href=\"__APP__/{$maval['inbindaname']}/miniindex/viewtype/view/bindrdid/{$vo['id']}/minitype/1/bindaname/{$actionname}/bindtype/{$maval['bindtype']}{$mainBindActionCondition}/bindid/{$bindid}/urdataroamid/{$maval['urdataroamid']}\"";
                                } else {
                                    $html .= $cstr." href=\"__APP__/{$maval['inbindaname']}/view/viewtype/view/id/{$maval['id']}/bindrdid/{$vo['id']}{$mainBindActionCondition}/urdataroamid/{$maval['urdataroamid']}\"";
                                }
                            }
 ...
 * @param string $action            
 * @author quqiang
 * @return int @date 2015年5月4日 下午4:26:28
 * @throws NullDataExcetion
 */
...
//		    if($vo['operateid'] == 1 && $vo['auditState'] == 3){
//				$VoOperateid=1;
//				$vo['operateid'] =0;
//			}
                    if ($formtype == 1) {
...
EOF;
		...
		if($VoOperateid!=$vo['operateid']){
			$vo['operateid'] =$VoOperateid ;
		}
                if ($oprateType == 'view') {
                    // 生成变更按钮
...
//                }
		...
		<ul class="right top_tool_bar" style="margin-right:0px;">
			<li class="left" style="{$taobiaoHtmlTemp}">
                <a class="allSaveBtn" href="javascript:void(0);" onclick="ConfirmCommit1(this.form,'__MODULE__');" ><span class="icon-save"></span> 保存</a>
            </li>
			{$appendBtnHtml}
		</ul>
<script>
    ...
    if (! empty($data)) {
        ...
        if ($formFiledList) {
            ...
            if (is_array($formFiledListArr) && count($formFiledListArr)) {
                ...
                $fd = $properModel->where($properMap)
                    ->field('fieldname,id')
                    ->select();
                if ($fd) {
                    foreach ($fd as $k => $v) {
 ...
 * 
 * @param string $souceStr
 *            mis_auto_cbj.orderno='010101' ， mis_auto_cbj.id in('1','2','3','4','56','67'), mis_auto_cbj.id >='2' and mis_auto_cbj.id <='10'
 * @param string $checkArr
 *            , 如： array('id'=>5 , 'orderno'=>'010101');
 * @author quqiang
