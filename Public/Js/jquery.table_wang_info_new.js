var TABLEWNEW = function() {
    ...
    var table_wang_new = {
        st: {
            ...
            importUrl:"",
            isSelectChange:true,
            istwodimensional:0,//是否二维录入，0表否，1表是
            istwodimensionalshow:0,//是否二维显示，0表否，1表是
            isinitth:true , //初始化列
            distinguish:"",  //是否能进行二维添行
            isaddrowimport:"",//是否显示新增行和导入按钮，默认1表示显示，0表示不显示
        },
        ...
        ThType: function(obj,column,val,dtdata) {
            var that = this;
            try{
            ...
            switch (template_key) {
                ...
                case "input":
                    ...
                    // alert(_ewdata[0]);
                    // alert(_ewdata[4]);
                    // for(var i=0;i<_ewdata[5].length;i++){
                    //     alert(_ewdata[5][i].fieldname);
                    // }
                    // alert(_ewdata[3]);
                    // }
                ...
                case "uploadfilenew": //url:__URL__/DT_uploadnew
                    ...
                    if(!is_readonly){
                        ...
                        html += '<a onclick="DTopenFile(this)" rel_name="'+template_name+'" rel_url="'+template_data.url+'" rel_index="#index#" id="DT_upload_'+column+'#index#" href="javascript:;" style="padding:3px 10px;" class="dt_upload_zero" title="附件管理">附件管理(<span class="attached_count">0</span>)</a>';
                        html += '</div>';
                    }
                    html += '</div>';
                    break;
...
//          $(tb).parents("form").find("[name='"+bindhz+"']").val(parseFloat(all_sum.toFixed(decimals))).trigger("change");
            //不同浏览器toFixed方法有误差
            var fixNum = new Number(sum+1).toFixed(decimals);//四舍五入之前加1
            var fixedNum = new Number(fixNum - 1).toFixed(decimals);//四舍五入之后减1，再四舍五入一下
            $(tb).find(".into_table_tj_"+index).html(parseFloat(sum.toFixed(decimals)));
...
//                  new_tr.find("td").eq(j).find('.list_group_lay').css("width",parseInt(td_width)+"px");
        ...
        addRow:function(rows){
            var that = this;
            var cls = $(that.st.tableId).attr('class');
            var clss = cls.split(" ");
            var inputid = $(that.st.tableId).attr('distinguish');
            if(clss[2] == 'istwodimensional' ||clss[2] == 'noistwodimensional'){
                if($("#"+inputid).val()){
        ...
        getTjColumnAll:function(index,data,type) {
            ...
            var decimals = $(tj_class).attr("decimals");
            if(type=="page"){
            ...
            //2016-5-4 按农担要求不进行四舍五入
            $(tb).find(tj_class).html(parseFloat(sum.toFixed(decimals)));
        },
        ...
        addRowHtml:function(){
            ...
            var isaddrowimport = $(that.st.table).find('.into_table_new').attr('isaddrowimport');
            if(isaddrowimport){
                if(isaddrowimport==1){
...
function initTableWNEW(dtdata,namses,tag) {
    var $curpanelobj=navTab.getCurrentPanel();
    //二维表
    $curpanelobj.find('.istwodimensional').each(function(){
        var this_obj = this;
        var role = $(this_obj).attr("role");
        if(!role && this_obj.offsetHeight>0 || dtdata){
            ...
            if(table_type=="list"){
                ...
                if($(this_obj).attr('distinguish') == tag){
                    table.st.scrollX = (width-14)+"px";
                    ...
                    if(dtdata){
                        ...
                        {
                            var _firsttr =  $(table.st.table).find("thead:eq(0)>tr:first").find("th:last");
                            var _th='';
                            for(var i=0;i<dtdata.length;i++){
                                _th = $("<th template_key='input' class='delete' template_name="+namses+"["+dtdata[i].zhibiaomingchen+"_"+dtdata[i].zhibiaobianma+"]"+">"+dtdata[i].zhibiaomingchen+"</th>");
                                _firsttr.before(_th);
                            }
                            table.st.isinitth =false;
                            table.st.distinguish = tag;
                            var nbox = navTab.getCurrentPanel();
                            nbox.find('.pageFormContent').eq(0).animate({scrollTop: $(table.st.table).offset().top}, 0);
                        }
                    }
                    //初始化表
                    table.init();
