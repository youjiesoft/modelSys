...
$(function(){
    ...
    var j=1;
    $("div.nbm_relation_form_tabs_navi .tabsHeaderContent:visible",anchorbox).each(function(){
        var name = modelname+"_"+j;
        $('a').remove('.nbm_relation_form_tabs_navi'+name);
        $(this).eq(0).find("a span").after("<a name='nbm_relation_form_tabs_navi"+name+"' class='nbm_relation_form_tabs_navi"+name+"'></a>");
        var relationval = $(this).eq(0).find("ul:first>li:first").find("a span").text();
        html += '<li> <a href="#nbm_relation_form_tabs_navi'+name+'">';
        html +=     '<span class="icon-paperclip"></span>';
        html +=     '<span class="inside_pages_btn_word" title="'+relationval+'">'+relationval+'</span>';
        html += '</a>';
        html += '</li>';
