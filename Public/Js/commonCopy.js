...
var useralert = setInterval(selectTime,100000000000);
function selectTime(){
	//读取数据库时间
	$.ajax({
		type: "POST",
...
function ajaxForUserData(){
	...
	var options = {};
	options.param = {ulid:ulid,data:data};
	options.width = 600;
	options.height = 570;
	options.mask = true;
...
function quickSearchChange($this,$model){
	$changeDiv = $($this).parent().parent().parent().next();
...
function fileexport(obj){
	...
	if(id){
		...
		$(".export_operate").offset({left:left});
		$(".export_operate").mouseleave(function(){
			$(".export_operate").hide();
		});
		$(".export_operate a").each(function(i){
...
function lookup_counter_check(json , obj){
	...
	//console.log(json);
	try{
		if(lpkey && lpfor && lpself && json[lpself]){ // && !isNullorEmpty(c.find('[class^="'+lpfor+'"]').val()) ) {
			//console.log('当前项的值：'+json[lpself]+' 并且需要更新其它值');
			$.ajax({
				url:TP_URL+'/lookupCounterCheck',
				type:'post',
				data:{'looukupkey':lpkey,'key':lpself,'val':json[lpself]},
