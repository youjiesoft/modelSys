...
function updateinvoicetype(obj, url, ids, params) {
    ...
    $.ajax({
        type: 'POST', dataType: "json", url: url, cache: false, data: arr,
...
function refreshsub_edit_sub_part_close(json) {
    ...
    $("#" + model + "edit_sub_part").loadUrl(TP_APP + "/" + model + "/edit/id/" + masid, {
        "refreshsub": 1,
        "height": h
    }, function () {
        ...
        if (show == 'none') {
            $.get(TP_APP + '/MisTodo/lookupshow', function (data) {
                ...
                $('#MisTodoShow a').bind('click', function (e) {
                    var __this = $(this);
                    navTab.openTab(__this.attr('tabid'), __this.attr('taburl'), {
                        title: __this.attr('tabname'),
                        fresh: true,
                        data: null
                    });
                });
...
function subViewChange($model, $masid, $method, $auditstatus) {
    ...
    var h = $("#" + $divid).height();
    $("#" + $divid).loadUrl(TP_APP + "/" + $model + "/" + $method + "/id/" + $masid, {
        "refreshsub": 1,
        "height": h,
...
function saveAndAdd(form, m, isedit) {
    ...
    return validateCallback(form, function (json) {
        ...
        if (isedit == 1) setTimeout(function () {
...
function nbm_datatable_callback(json, obj) {
    try {
        ...
        var nval=JSON.parse($(obj).attr('nval'));
        console.log(dtdata);
        ...
        if (typeof(dtdata) == 'object' && isNullorEmpty(relation)) {
            ...
            console.log(nval);
            //是否二维表录入判断
            if (nval.istwodimensional==1) {
                initTableWNEW(dtdata,nval.namses,nval.orgTag);
            }else{
...
Date.prototype.DateDiff = function (strInterval, dtEnd, dtStart) {
    ...
    switch (strInterval) {
        case 's' :
            return parseInt(parseFloat((dtEnd - dtStart) / 1000) * jindu) / jindu;
        case 'i' :
            return parseInt(parseFloat((dtEnd - dtStart) / 60000) * jindu) / jindu;
        case 'h' :
            return parseInt(parseFloat((dtEnd - dtStart) / 3600000) * jindu) / jindu;
        case 'd' :
            return parseInt(parseFloat((dtEnd - dtStart) / 86400000) * jindu) / jindu;
        case 'w' :
            return parseInt(parseFloat((dtEnd - dtStart) / (86400000 * 7)) * jindu) / jindu;
        case 'm' :
...
//附件上传特殊处理，先进入编辑状态后才能正常保存附件
function Fujianclick(obj) {
    var rel_type = $(obj).parent().parent().parent().parent().find(".save_row_btn").attr("rel_type");
    if(rel_type=="edit"){
        $(obj).parent().parent().parent().parent().find(".save_row_btn").click();
    }
    DTopenFile(obj);
    return false;
}
...
function DTopenFile(obj) {
    $this = $(obj);
    //通过url参数传递判断是否显示上传附件按钮
    var rel_type1 = $this.parents("tr").find(".save_row_btn").attr("rel_type");
    if(rel_type1 == "save"){
        var list_group_lay=$this.parents("tr").find("a[title='附件管理']").attr("rel_url")+'/shangchuan/1';
        $this.parents("tr").find("a[title='附件管理']").attr("rel_url",list_group_lay);
    }
    var title = $this.attr("title") || $this.text();
    var rel = $this.attr("rel") || "_blank";
    ...
    options.param = {
        ...
        rel_subid: rel_subid,
        rel_index: rel_index,
        rel_tableid: rel_tableid,
        rel_name: rel_name,
        rel_tablename: rel_tablename,
        rel_fieldname: rel_fieldname
    };
    $.pdialog.open(url, rel, title, options);
}
...
//                        alert("身份证号码不符合国定标准，请核对！");
                        $(this).addClass('error')
                        $(this).after('<span for="'+$(this).attr('name')+'" generated="true" class="error" title="请输入合法的数字">' + '请输入合法的数字' + '</span>');
                        //$("#person_id").val("");
                        $("#birthday").val("")
                        return;
                    }
