...
function initUI(_box) {
	...
	$("a.js-inputCheckMap", $p).on("click", function () {
		...
		var searchAddress = $('input[name$="[detail]"]', context).val();
		var $target = {};
		$target['s_address'] = $('input[name$="[address]"]', context);
		$target['s_coordinatex'] = $('input[name$="[coordinatex]"]', context);
		$target['s_coordinatey'] = $('input[name$="[coordinatey]"]', context);
		var FieldNames = ['province', 'city', 'area', 'town', 'street', 'longitude', 'latitude'];
		$.each(FieldNames, function (index, value) { $target[value] = $(`input[name$="[${value}]"]`, context); });
		var mapId = 'gddt';
		...
		function initMap(AMap, mapId, inputId, searchAddress, $target) {
			...
			//数据加载完毕后,关闭加载动画，显示地图
			document.querySelector('#' + inputId).value = searchAddress?searchAddress:'';
	...
	//详细地址改变的时候查询经纬度
	$('input[name$="[address]"]', $p).on('blur', function () {
		console.log(1);
		var context = $(this).parent('.address_elm').parent('[category="areainfomap"]');
		var _this = $(this);
		var fieldNames = ['province', 'detail', 'province', 'city', 'area', 'town', 'street', 'longitude', 'latitude'];
		var addressObj = {};
		$.each(fieldNames, function (index, value) { addressObj[value] = $(`input[name$="[${value}]"]`, context); });
		var flag = false;
