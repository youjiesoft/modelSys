...
$('div.fieldset_show_box',$p).on("click",function(){
var isClass = $(this).find(".fieldset_legend_toggle").hasClass("fieldset_toggle_down");
    if(isClass){
		$(this).find(">div").hide();
	}else{
		$(this).find(">div").show();
...
if($.fn.select2)$("select.select2",$p).select2();
...
complete:function(res){
	$("#userConstantly_div").loadUrl(TP_APP+"/UserOftenMenu/oftenlist");
},
error:DWZ.ajaxError});
},
...
if($(this).prop('class').toString().indexOf('selected')>-1){
	$(this).find('td:first').find(':checkbox').attr('checked', true);
}
/* modify end */
var sTarget=$tr.attr("target");
if(sTarget){
...
var mulitlook_not_reset=$box.find(":input[name='multi_lookup_not_reset']").length;
if(mulitlook_not_reset) mulitlook_reset=0;
$box.find(":input").each(function(){
var $input=$(this),inputName=$input.attr("name"),callbackinput=$input.attr('callback');
for(var key in args){
...
if( $.trim($input.val())!="" && mulitlook_reset==0){break;}
/*-------lookup搜索修改END-------------*/
if($input.parent("div").hasClass("select")){
